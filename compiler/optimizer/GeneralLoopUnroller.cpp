...
public:
    LoopWeightProbe(TR::Compilation *comp)
        ...
        , _numPureFunctions(0)
        , _numBIFs(0)
        , _used(comp->allocator())
        , _defd(comp->allocator())
    {}
...
static TR::ILOpCodes getConstOpCode(TR::DataType type)
{
    switch (type.getDataType()) {
        ...
        case TR::Int64:
            return TR::lconst;
        case TR::Address:
            return TR::comp()->target().is64Bit() ? TR::lconst : TR::iconst;
        default:
...
static TR_ScratchList<TR::CFGEdge> *findCorrespondingCFGEdges(TR_Structure *from, TR_Structure *to,
    TR::Compilation *comp)
{
    if (from->asBlock()) {
...
{
    TR::Block *block
        = subNode->getStructure()->asBlock() != NULL ? subNode->getStructure()->asBlock()->getBlock() : NULL;
    return !block || loop->getEntryBlock()->getFrequency() < 5000 || block->getFrequency() > 2000
        || block->getFrequency() == -1;
}
...
void TR_LoopUnroller::unrollLoopOnce(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode, bool finalUnroll)
{
    ...
    for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
        ...
        TR_StructureSubGraphNode *clonedSubNode
            = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(clonedSubStruct);
    ...
    for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
        ...
        for (auto edge = subNode->getSuccessors().begin(); edge != subNode->getSuccessors().end(); ++edge) {
            ...
            } else if (subNode != branchNode && !clonedSucc) // Type B
            {
                ...
                if (finalUnroll && (_unrollKind == CompleteUnroll) && subNodeHasBackEdge)
            ...
            } else if (succ == loop->getEntry()) // Type C
            {
                ...
                if (finalUnroll) {
                    if (_unrollKind != CompleteUnroll)
                        ...
                        addEdgeAndFixEverything(loop, *edge, clonedSubNode, loop->getEntry(), false, false, true,
                            BackEdgeToEntry);
                    else {
...
void TR_LoopUnroller::redirectBackEdgeToExitDestination(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode,
    TR_StructureSubGraphNode *fromNode, bool notLoopBranchNode)
{
    // Find the exit destination
    TR::CFGEdge *exitEdge = NULL;
    for (auto edge = branchNode->getSuccessors().begin();
         (edge != branchNode->getSuccessors().end()) && exitEdge == NULL; ++edge) {
        if (loop->isExitEdge(*edge))
            exitEdge = *edge;
    }

    // Add A->X*
    addEdgeForSpillLoop(loop, exitEdge, fromNode,
        findNodeInHierarchy(loop->getParent()->asRegion(), toStructureSubGraphNode(exitEdge->getTo())->getNumber()),
        false, BackEdgeFromLastGenerationCompleteUnroll, notLoopBranchNode);
}

void TR_LoopUnroller::modifyBranchTree(TR_RegionStructure *loop, TR_StructureSubGraphNode *loopNode,
    TR_StructureSubGraphNode *branchNode)
{
    ...
    TR_StructureSubGraphNode *predNode = toStructureSubGraphNode(loopNode->getPredecessors().front()->getFrom());
    ...
    if (pBlock->getEntry() == 0) {
        TR::Block *newBlock = TR::Block::createEmptyBlock(branch, comp(), MAX_COLD_BLOCK_COUNT + 1,
            pBlock); // branchBlock->getFrequency());
        newBlock->getExit()->join(loop->getEntryBlock()->getEntry());
        comp()->setStartTree(newBlock->getEntry());

        _cfg->addNode(newBlock);
        TR_BlockStructure *newBlockStr
            = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
    ...
    int32_t adjustment = getLoopStride() * (1 + _unrollCount) + (isIncreasingLoop() ? -1 : 1);
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && getTestIsUnsigned()) {
        // Address compare generated by loop strider does not have constant node,
        // thus should not fall into this category
        TR_ASSERT(!originalLimitNode->getType().isAddress(),
            "Counted loop with a constant address limit node is unexpected, test node is %p", getTestNode());
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
        ...
        if (getIndexType().isAddress()) {
            ...
            tempSym
                = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), getTestChildType().getDataType());
        ...
        TR::ILOpCodes addOpCode = getIndexType().isAddress() ? (is64BitComparison ? TR::aladd : TR::aiadd)
        ...
        storeOpCode = getIndexType().isAddress() ? TR::astore : (is64BitComparison ? TR::lstore : TR::istore);
        //         }
        ...
        if (getIndexType().isAddress() || getTestChildType().isAddress()) {
            TR_ASSERT(getIndexType().isAddress() && getTestChildType().isAddress(),
                "Test child type and IV type should both be address");
        } else if (newLimitNode->getType().isAggregate() && !getTestChildType().isAggregate()) {
            TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(newLimitNode->getDataType(),
                getTestChildType().getDataType(), true);
            TR_ASSERT(o2xOp != TR::BadILOp, "could not find conv op for %s -> %s conversion\n",
                TR::DataType::getName(newLimitNode->getDataType()),
                TR::DataType::getName(getTestChildType().getDataType()));
        ...
        if (lastTree->getNode()->getOpCode().isBranch()) {
            ...
            TR::Block *newBlock
                = TR::Block::createEmptyBlock(lastTree->getNode(), comp(), pBlock->getFrequency(), pBlock);
            ...
            TR_BlockStructure *newBlockStructure
                = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
            TR_StructureSubGraphNode *newSubNode
                = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(newBlockStructure);
        ...
        pBlock->append(
            TR::TreeTop::create(comp(), TR::Node::createif(opCode, originalLimitClone, c2, spillBlock->getEntry())));
    ...
    if (1) /// moveEntryNode
    {
        TR::ILOpCodes opCode = _branchToExit ? branch->getOpCodeValue() : /* use same polarity */
        ...
        if (getIndexType().isAddress() || getTestChildType().isAddress()) {
            ...
            TR::ILOpCodes o2xOp
                = TR::ILOpCode::getProperConversion(firstChild->getDataType(), getTestChildType().getDataType(), true);
            TR_ASSERT(o2xOp != TR::BadILOp, "could not find conv op for %s -> %s conversion\n",
                TR::DataType::getName(firstChild->getDataType()),
                TR::DataType::getName(getTestChildType().getDataType()));
        ...
        TR::TreeTop *iterGuard
            = TR::TreeTop::create(comp(), TR::Node::createif(opCode, firstChild, dupNewLimit, spillBlock->getEntry()));
...
void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode)
{
        ...
        if (!loop->isExitEdge(current)) {
            ...
            if (predNode->getNumber() < _numNodes) {
                if (shouldConnectToNextIteration(predNode, loop)) {
                    addEdgeAndFixEverything(loop, current, predNode, _firstEntryNode, true, true, true,
    ...
    } else if (isCountedLoop() && _unrollKind != CompleteUnroll) /*GGLU*/
    {
        ...
        TR_ASSERT(branch->getOpCodeValue() != TR::ifacmpeq && branch->getOpCodeValue() != TR::ifacmpne,
            "Counted loop with branch node %p of opcode %s is unexpected\n", branch, branch->getOpCode().getName());
    ...
    } else if (_unrollKind == CompleteUnroll) /*GGLU*/
    {
        ...
        if (loop->contains(destStruct, loop->getParent())) {
            ...
            for (auto edge = branchNode->getSuccessors().begin();
                 (edge != branchNode->getSuccessors().end()) && exitNode == NULL; ++edge) {
                if (loop->isExitEdge(*edge))
                    exitNode = (*edge)->getTo();
            }
            ...
            TR::Block *blockX = getEntryBlockNode(concreteExitNode)->getStructure()->asBlock()->getBlock();
    ...
    if (_spillLoopRequired) {
        ...
        if (1) {
            ...
            TR::Block *newIfBlock = TR::Block::createEmptyBlock(branchTreeTop->getNode(), comp(),
                origExitBlock->getFrequency(), origExitBlock);
            ...
            if (1) {
                ...
                if (1) {
                    ...
                    if (getIndexType().isAddress() || getTestChildType().isAddress()) {
                        ...
                        TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(c1->getDataType(),
                            getTestChildType().getDataType(), true);
                        TR_ASSERT(o2xOp != TR::BadILOp, "could not find conv op for %s -> %s conversion\n",
                            TR::DataType::getName(c1->getDataType()),
                            TR::DataType::getName(getTestChildType().getDataType()));
                    ...
                    newIfTree = TR::TreeTop::create(comp(),
                        TR::Node::createif(ifNode->getOpCodeValue(), c1, boundNode, ifNode->getBranchDestination()));
                }

                newIfBlock->append(newIfTree);
                _cfg->addNode(newIfBlock);
                newIfBlockStructure = new (_cfg->structureMemoryRegion())
                    TR_BlockStructure(comp(), newIfBlock->getNumber(), newIfBlock);
                ...
                TR::Block *gotoBlock = TR::Block::createEmptyBlock(gotoNode, comp(), origExitBlock->getFrequency(),
                    origExitBlock); // spillLoop->getEntryBlock()->getFrequency());
        ...
        for (auto edge = loopNode->getSuccessors().begin();
             (edge != loopNode->getSuccessors().end()) && !correspondingExitEdge; ++edge)
        ...
        if (1) {
            ...
            if (!e) // there do not remain an edge in L going to X
            {
                if (exitNode->getStructure()->getParent() == parent) {
                    bool found = (std::find(loopNode->getSuccessors().begin(), loopNode->getSuccessors().end(),
                                      correspondingExitEdge)
                        != loopNode->getSuccessors().end());
                    loopNode->getSuccessors().remove(correspondingExitEdge);
                    TR_ASSERT(found, "GLU: bad structure while removing edge");
                    found = (std::find(exitNode->getPredecessors().begin(), exitNode->getPredecessors().end(),
                                 correspondingExitEdge)
                        != exitNode->getPredecessors().end());
...
void TR_LoopUnroller::removeExternalEdge(TR_RegionStructure *parent, TR_StructureSubGraphNode *from, int32_t to)
{
    ...
    if (numExitEdgesTo(from->getStructure()->asRegion(), to) == 0) {
        bool found = (std::find(relevantEdge->getFrom()->getSuccessors().begin(),
                          relevantEdge->getFrom()->getSuccessors().end(), relevantEdge)
            != relevantEdge->getFrom()->getSuccessors().end());
        relevantEdge->getFrom()->getSuccessors().remove(relevantEdge);
        TR_ASSERT(found, "GLU: bad structure while removing edge");
        found = (std::find(relevantEdge->getTo()->getPredecessors().begin(),
                     relevantEdge->getTo()->getPredecessors().end(), relevantEdge)
            != relevantEdge->getTo()->getPredecessors().end());
...
void TR_LoopUnroller::collectInternalPointers()
{
    ...
    for (TR::Block *preheader = it.getFirst(); preheader; preheader = it.getNext()) {
        ...
        while (currentTree != exitTree) {
            ...
            if (node->getOpCodeValue() == TR::astore && node->getSymbol()->isAuto()
                ...
                if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub())
                    ...
                    if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
                        ...
                        if (firstChild->getOpCode().isLoad()) {
                            ...
                            if (isBiv(_loop, firstChild->getSymbolReference(), biv))
                                canMatch = true;
                            else if (isBiv(_loop, node->getSymbolReference(), biv)
                                && firstChild->getOpCode().isLoadVarDirect() && firstChild->getSymbol()->isAuto()
                                && node->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect()
                                && node->getFirstChild()->getFirstChild()->getSymbol()->isAuto()
                                && node->getFirstChild()
                                       ->getFirstChild()
                                       ->getSymbol()
                                       ->castToAutoSymbol()
                                       ->isPinningArrayPointer())
                                canMatchAsIP = true;
...
void TR_LoopUnroller::examineNode(TR::Node *node, intptr_t visitCount)
{
    ...
    if (symbol && symbol->isArrayShadowSymbol()) {
        ...
        if (!listOfAA) {
            ListsOfArrayAccesses *loaa
                = (ListsOfArrayAccesses *)trMemory()->allocateStackMemory(sizeof(ListsOfArrayAccesses));
        ...
        if (node->getFirstChild()->getOpCodeValue() == TR::aload && node->getFirstChild()->getSymbol()->isAuto()
...
bool TR_LoopUnroller::haveIdenticalOffsets(IntrnPtr *intrnPtr1, IntrnPtr *intrnPtr2)
{
    ...
    if (intrnPtr1->biv == intrnPtr2->biv && intrnPtr1->offsetValue == intrnPtr2->offsetValue)
        return true;
    ...
    int64_t entryValue1
        = entryValueNode1->getType().isInt64() ? entryValueNode1->getLongInt() : (int64_t)entryValueNode1->getInt();
    int64_t entryValue2
        = entryValueNode2->getType().isInt64() ? entryValueNode2->getLongInt() : (int64_t)entryValueNode2->getInt();
...
bool TR_LoopUnroller::isSymRefSameTypeArrayShadow(TR::Node *node)
{
    ...
    return (node->getSymbolReference()->getReferenceNumber()
        == comp()->getSymRefTab()->getArrayShadowIndex(node->getDataType()));
}
...
void TR_LoopUnroller::refineArrayAliasing()
{
    ...
    for (ListsOfArrayAccesses *loaa = it.getFirst(); loaa; loaa = it.getNext()) {
        ...
        if (aa) {
            newSymRef = comp()->getSymRefTab()->createRefinedArrayShadowSymbolRef(
                aa->aaNode->getSymbolReference()->getSymbol()->getDataType());
...
TR::Node *TR_LoopUnroller::createIfNodeForSpillLoop(TR::Node *ifNode)
{
    ...
    if (getIndexType().isAddress() || getTestChildType().isAddress()) {
        ...
        TR::ILOpCodes o2xOp
            = TR::ILOpCode::getProperConversion(c1->getDataType(), getTestChildType().getDataType(), true);
        TR_ASSERT(o2xOp != TR::BadILOp, "could not find conv op for %s -> %s conversion\n",
    ...
    return TR::Node::createif(ifNode->getOpCodeValue(), c1, ifNode->getSecondChild()->duplicateTree(),
        ifNode->getBranchDestination());
}

void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode)
{
    // Reset the mappers.
    _iteration = 0;
    memset(_blockMapper[CURRENT_MAPPER], 0, _numNodes * sizeof(TR::Block *));
...
void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop,
   ...
   TR_BlockStructure *newBranchStructure = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(),
newBranchBlock->getNumber(), newBranchBlock); TR_StructureSubGraphNode *newBranchNode = new
(_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(newBranchStructure); spillLoop->addSubNode(newBranchNode);
...
TR_StructureSubGraphNode *TR_LoopUnroller::findNodeInHierarchy(TR_RegionStructure *region, int32_t num)
{
    if (!region)
        return NULL;
...
void TR_LoopUnroller::addEdgeForSpillLoop(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *newToNode, bool removeOriginal,
    EdgeContext edgeContext, bool notLoopBranchNode)
{
    ...
    TR_ScratchList<TR::CFGEdge> *cfgEdges
        = findCorrespondingCFGEdges(fromNode->getStructure(), toNode->getStructure(), comp());
    ...
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
        ...
        if (lastNode->getOpCode().isBranch() && (lastNode->getBranchDestination() == to->getEntry())
            ...
            if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
                newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), to->getEntry(), newTo->getEntry());
            } else {
        ...
        // FALLS INTO
            ...
            if (newFromNextBlock != newTo) // but the copy does not fall through
            {
                ...
                if (notLoopBranchNode && (edgeContext == BackEdgeFromLastGenerationCompleteUnroll)) {
                    ...
                    if (newFromLastNode->getOpCode().isBranch()) {
                        ...
                        /*
                           ...
                           {
                           newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), newTo->getEntry(),
                        gotoBlock->getEntry());
                           }
                        */
                    }
                ...
                if (edgeContext == BackEdgeFromLastGenerationCompleteUnroll) {
                    if (newFrom->getLastRealTreeTop()->getNode()->getOpCode().isBranch())
                        newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(), oldDest,
                            gotoBlock->getEntry());
                }

                // create the structures
                TR_BlockStructure *gotoStruct
                    = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock);
                TR_StructureSubGraphNode *gotoSubNode
                    = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(gotoStruct);
...
void TR_LoopUnroller::addExitEdgeAndFixEverything(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *exitNode, TR::Block *newExitBlock,
    EdgeContext context)
{
    TR_StructureSubGraphNode *fromNode = toStructureSubGraphNode(originalEdge->getFrom());
            ...
            if (newFromNextBlock != newTo) {
                ...
                if (context == ExitEdgeFromBranchNode) {
                    ...
                    if (!cfgEdgeAlreadyExists(newFrom, newTo)) {
                        ...
                        gotoNode->setLocalIndex(CREATED_BY_GLU);
                        TR::Block *gotoBlock
                            = TR::Block::createEmptyBlock(lastNode, comp(), newTo->getFrequency(), newTo);
                        gotoBlock->append(gotoTree);
                        gotoBlock->getEntry()->getNode()->setLocalIndex(CREATED_BY_GLU);
                        _cfg->addNode(gotoBlock);
                        ...
                        // create the structures
                        TR_BlockStructure *gotoStruct = new (_cfg->structureMemoryRegion())
                            TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock);
                        TR_StructureSubGraphNode *gotoSubNode
                            = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(gotoStruct);
                        region->addSubNode(gotoSubNode);
...
void TR_LoopUnroller::addEdgeAndFixEverything(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *newToNode, bool redirectOriginal,
    bool removeOriginalEdges, bool edgeToEntry, EdgeContext context)
{
    TR_StructureSubGraphNode *fromNode = toStructureSubGraphNode(originalEdge->getFrom());
    TR_StructureSubGraphNode *toNode = toStructureSubGraphNode(originalEdge->getTo());
    ...
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
        ...
        // FALLS INTO
            ...
            TR::Block *newFromNextBlock = newFrom->getNextBlock();
            if (newFromNextBlock != newTo) {
                TR_ASSERT(context == BackEdgeFromPrevGeneration || context == BackEdgeToEntry,
...
bool TR_LoopUnroller::cfgEdgeAlreadyExists(TR::Block *from, TR::Block *to, EdgeContext edgeContext)
{
    for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge) {
        ...
        if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
            ...
            if (firstRealTree == lastRealTree && firstRealTree->getNode()->getOpCodeValue() == TR::Goto) {
                ...
                //
                if (dest->getNumber() == to->getNumber() && lastRealTree->getNode()->getLocalIndex() == CREATED_BY_GLU
...
bool TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode *from, TR_StructureSubGraphNode *to)
{
    for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge)
...
bool TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode *from, int32_t toNum)
{
    for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge)
...
{
    if (s->asRegion())
        return cloneRegionStructure(s->asRegion());
...
{
    TR_RegionStructure *clonedRegion
        = new (_cfg->structureMemoryRegion()) TR_RegionStructure(comp(), 0xDeadF00d); // for now
    ...
    for (subNode = si.getFirst(); subNode; subNode = si.getNext()) {
        ...
        TR_ASSERT(subNode->getExceptionSuccessors().empty(),
            "should not reach here - we dont unroll loops with exceptions");
    }
...
void TR_LoopUnroller::processSwingBlocks(TR::Block *from, TR::Block *to)
{
    ...
    if (pF == NULL || isSuccessor(pF, from) == false) { // insert F between pT and T
    ...
    } else if (nT == NULL || isSuccessor(to, nT) == false) { // insert T between F and nF
...
void TR_LoopUnroller::fixExitEdges(TR_Structure *s, TR_Structure *clone, TR_StructureSubGraphNode *branchNode)
{
    if (s->asBlock())
        return;
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        TR_StructureSubGraphNode *clonedSubNode
            = findNodeInHierarchy(clonedRegion, _blockMapper[CURRENT_MAPPER][subNode->getNumber()]->getNumber());
        fixExitEdges(subNode->getStructure(), clonedSubNode->getStructure());
    }
    ...
    for (edge = ei.getFirst(); edge; edge = ei.getNext()) {
        if (region->isExitEdge(edge)) {
            ...
            TR_StructureSubGraphNode *clonedFromNode
                = findNodeInHierarchy(clonedRegion, _blockMapper[CURRENT_MAPPER][fromNode->getNumber()]->getNumber());
            ...
            {
                TR_StructureSubGraphNode *exitNode = findNodeInHierarchy(region->getParent()->asRegion(), toNum);
                addExitEdgeAndFixEverything(clonedRegion, edge, clonedFromNode, exitNode, clonedBlock, context);
            } else {
                TR_StructureSubGraphNode *exitNode = findNodeInHierarchy(region->getParent()->asRegion(), toNum);
                addExitEdgeAndFixEverything(clonedRegion, edge, clonedFromNode, exitNode, NULL, context);
            }
        }
    }
    processSwingQueue();
...
void TR_LoopUnroller::prepareLoopStructure(TR_RegionStructure *loop)
{
    TR_ScratchList<TR::Block> blocksInRegion(trMemory());
    loop->getBlocks(&blocksInRegion);
...
bool TR_LoopUnroller::isTransactionStartLoop(TR_RegionStructure *loop, TR::Compilation *comp)
{
    if (comp->getOption(TR_DisableTLE)
        || !comp->cg()->getSupportsTM()) // if TLE is not supported or on, it is not possible
        return false;
...
bool TR_LoopUnroller::isWellFormedLoop(TR_RegionStructure *loop, TR::Compilation *comp, TR::Block *&loopInvariantBlock)
{
    ...
    for (TR::Block *block = blocks.getFirst(); block; block = blocks.getNext()) {
        if (block->hasExceptionSuccessors() || block->hasExceptionPredecessors())
            return false;
    }
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer, TR_RegionStructure *loop,
    ...
    , _wasEQorNELoop(false)
    , _origLoopCondition(TR::BadILOp)
    , _newSymRefs(c->trMemory())
    , _startPosOfUnrolledBodies(NULL)
    , _endPosOfUnrolledBodies(NULL)
    , _listOfInternalPointers(c->trMemory())
    , _listOfListsOfArrayAccesses(c->trMemory())
{
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer, TR_RegionStructure *loop,
    ...
    TR::Block *invariantBlock, int32_t vectorSize)
    : _comp(c)
    , _trMemory(c->trMemory())
    , _optimizer(optimizer)
    , _loop(loop)
    ...
    , _piv(piv)
    , _spillNode(0)
    , _overflowTestBlock(0)
    , _loopIterTestBlock(0)
    , _loopInvariantBlock(invariantBlock)
    , _loopInvariantBlockAtEnd(false)
    , _iteration(0)
    , _firstEntryNode(0)
    ...
    , _listOfListsOfArrayAccesses(c->trMemory())
{
    ...
    _branchToExit
        = !loop->contains(branch->getBranchDestination()->getNode()->getBlock()->getStructureOf(), loop->getParent());
}

// ::unrollCountedLoop
bool TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop, TR_PrimaryInductionVariable *piv,
    UnrollKind unrollKind, int32_t unrollCount, int32_t peelCount, TR::Optimizer *optimizer)
{
    ...
    if (1) {
        ...
        if (unroller._spillLoopRequired) {
            ...
            if (!backEdgeFound) {
                ...
                if (branchBlock == loop->getEntryBlock()) {
                    if ((branchBlock->getLastRealTreeTop() == branchBlock->getFirstRealTreeTop())
                        || ((branchBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() == TR::asynccheck)
                            && (branchBlock->getLastRealTreeTop()
                                == branchBlock->getFirstRealTreeTop()->getNextTreeTop())))
                    ...
                    if (blocksInLoop.isDoubleton() && nextBlock->getSuccessors().size() == 1) {
                        if ((nextBlock->getEntry()->getNextTreeTop() == nextBlock->getExit())
                            ...
                            || ((nextBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() == TR::asynccheck)
                                && (nextBlock->getLastRealTreeTop()
                                    == nextBlock->getFirstRealTreeTop()->getNextTreeTop())))
                            loopCanBeUnrolled = true;
                    }
                }
                ...
                if (loopCanBeUnrolled) {
                    ...
                    // Further the only code other from the entry block in the loop must be the branch (and an optional
                    // async check)
                    //
                } else {
                    dumpOptDetails(comp,
                        "Cannot unroll loop %d: exit condition is not in a block containing a backedge\n",
                        loop->getNumber());
                    return false;
                }
            }
        ...
        if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
            if (piv->getDeltaOnExitEdge() != 1 && piv->getDeltaOnExitEdge() != -1) {
            ...
            //
            //
            TR_ASSERT(!branch->getFirstChild()->getType().isAddress(),
                "Excpeting an integral compare with integral first child");
    ...
    if (!performTransformation(comp,
            "%sUnrolling counted loop %d [unrollfactor:%d, peelcount:%d, spill:%s completeunroll:%s]\n", OPT_DETAILS,
            loop->getNumber(), unrollCount + 1, peelCount, unrollKind == UnrollWithResidue ? "yes" : "no",
...
bool TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop, int32_t unrollCount, int32_t peelCount,
    TR::Optimizer *optimizer)
{
    ...
    TR_StructureSubGraphNode *branchNode
        = loop->getExitEdges().getListHead()->getData()->getFrom()->asStructureSubGraphNode();
    ...
    if (!branchStr) {
        ...
        dumpOptDetails(comp, "Cannot unroll loop %d: branchnode %d is not a block\n", loop->getNumber(),
            branchNode->getNumber());
        return false;
    }

    TR_LoopUnroller unroller(comp, optimizer, loop, branchNode, unrollCount, peelCount, loopInvariantBlock,
        GeneralUnroll);
...
int32_t TR_GeneralLoopUnroller::perform()
{
    ...
    if (comp()->getOption(TR_ProcessHugeMethods)) {
        budget = 100000;
    } else if (comp()->getMethodHotness() == hot || comp()->getMethodHotness() == veryHot) {
    ...
    for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
        ...
        if (weight > 0 && unrollCount > 0 && loopCanBeUnrolled) {
            UnrollInfo *info = new (trStackMemory()) UnrollInfo(loop, weight, cost, unrollKind, unrollCount, peelCount);
            Q.add(info);
        }
    }
    ...
    while (1) {
        ...
        if (top->_cost > budget) {
            dumpOptDetails(comp(), "Loop %d unroll cost %d > %d remaining budget\n", top->_loop->getNumber(),
                top->_cost, budget);
        ...
        if (top->_loop->getPrimaryInductionVariable()) {
            didUnroll = TR_LoopUnroller::unroll(comp(), top->_loop, top->_loop->getPrimaryInductionVariable(),
...
void TR_GeneralLoopUnroller::collectNonColdInnerLoops(TR_RegionStructure *region, List<TR_RegionStructure> &innerLoops)
{
...
void TR_GeneralLoopUnroller::gatherStatistics(TR_Structure *str, int32_t &numNodes, int32_t &numBlocks,
    int32_t &numBranches, int32_t &numSubscripts, LoopWeightProbe &lwp)
{
    if (str->asBlock()) {
        ...
        for (TR::TreeTop *tt = block->getFirstRealTreeTop(); tt != block->getExit(); tt = tt->getNextRealTreeTop())
...
void TR_GeneralLoopUnroller::countNodesAndSubscripts(TR::Node *node, int32_t &numNodes, int32_t &numSubscripts,
    LoopWeightProbe &lwp)
{
    if (!node)
        return;
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(List<TR_RegionStructure> &innerLoops)
{
    ListIterator<TR_RegionStructure> it(&innerLoops);
    for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
        if (loop->getPrimaryInductionVariable())
        ...
        if (!performTransformation(comp(),
                "%sInserting artificial counter in loop %d in order to profile the itercount\n", OPT_DETAILS,
                loop->getNumber()))
            continue;
        ...
        TR::Node *zero = TR::Node::create(firstNode, TR::lconst, 0);
        zero->setLongInt(0);
        TR::Node *one = TR::Node::create(firstNode, TR::lconst, 0);
        one->setLongInt(1);

        invarBlock->prepend(
            TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::lstore, 1, 1, zero, counterSymRef)));
        entryBlock->prepend(TR::TreeTop::create(comp(),
            TR::Node::create(TR::lstore, 1,
                TR::Node::create(TR::ladd, 2, TR::Node::createLoad(firstNode, counterSymRef), one))));

        int32_t postNum = loop->getExitEdges().getListHead()->getData()->getTo()->getNumber();
        TR_StructureSubGraphNode *postNode
            = TR_LoopUnroller::findNodeInHierarchy(loop->getParent()->asRegion(), postNum);
        TR::Block *postBlock = postNode->getStructure()->getEntryBlock();
        postBlock->prepend(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(TR::lstore, 1, 1, TR::Node::createLoad(firstNode, counterSymRef),
                counterSymRef)));
        // FIXME: complete - mark the node as needs to be profiled
    }
}

static bool exitsLoop(TR::Compilation *comp, TR_RegionStructure *loop, TR_StructureSubGraphNode *source)
{
    ...
    int32_t &unrollCount, int32_t &peelCount, int32_t &cost)
{
    ...
    dumpOptDetails(comp(), "\tnumNodes = %d, numBlocks = %d, entryBlockFreq = %d, numSubscripts = %d\n", numNodes,
        numBlocks, entryBlockFrequency, numSubscripts);
    ...
    if (trace()) {
        dumpOptDetails(comp(), "\tnumKilled = %u, numUsed = %u, numMatConst = %u, numUnmatConst = %u", lwp._numKilled,
            lwp._numUsed, lwp._numMatConst, lwp._numUnmatConst);
        dumpOptDetails(comp(), "\nUSE: ");
        (*comp()) << lwp._used;
        dumpOptDetails(comp(), "\nDEF: ");
        (*comp()) << lwp._defd;
    ...
    if (piv) {
        ...
        if (loop->getPrimaryInductionVariable()) {
            ...
            for (auto e = entryNode->getPredecessors().begin(); e != entryNode->getPredecessors().end(); ++e) {
                ...
                //
                if (!loop->contains(source->getStructure(), loop->getParent()) || exitsLoop(comp(), loop, source))
                    continue;
                ...
                if (source->getStructure()->asBlock()) {
                    ...
                    if (!branchNode->getOpCode().isGoto() && !branchContainsInductionVariable(loop, branchNode)) {
                        dumpOptDetails(comp(),
                            "backedge branch [%p] is controlled using a biv, so unrolling as non-counted\n",
                            branchNode);
                        loop->setPrimaryInductionVariable(NULL);
                        break;
                    }
    ...
    if (piv) {
            ...
            if (iterCount <= maxIterCountForCompleteUnroll
                ...
                if (!multipleOfPreferred && !multipleOf2 && !multipleOf3) {
                    ...
                    if (comp()->getOption(TR_Randomize)) {
                        unrollCount = (multipleOf4 ? 3 : multipleOf3 ? 2 : multipleOf2 ? 1 : 0);
        ...
        if (unrollKind != TR_LoopUnroller::CompleteUnroll && growth > maxSizeThreshold) {
        ...
        if (_haveProfilingInfo) {
            ...
            if (canUnrollUnCountedLoop(loop, numBlocks, numNodes, entryBlockFrequency)) {
                ...
                for (int32_t j = 3; j >= 1; --j)
                    if ((numNodes - numSubscripts * 0.5) * (j + 1)
                        <= hotnessFactor * maxSizeThresholdForNonCountedLoops) {
                        unrollCount = j;
                        break;
                    }
                spillLoop = 0;
...
bool TR_GeneralLoopUnroller::canUnrollUnCountedLoop(TR_RegionStructure *loop, int32_t numBlocks, int32_t numNodes,
    int32_t entryBlockFrequency)
{
    ...
    if ((numBlocks <= 1 && numNodes <= 13 && ((entryBlockFrequency * 200) > (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)))
        ...
        for (b = bilIt.getCurrent(); b; b = bilIt.getNext()) {
            ...
            for (TR::TreeTop *tt = entry; tt != exit; tt = tt->getNextRealTreeTop()) {
                ...
                if (node->getOpCodeValue() == TR::NULLCHK && (firstChild = node->getFirstChild())
                    && firstChild->getOpCode().isLoad()) {
...
bool TR_GeneralLoopUnroller::branchContainsInductionVariable(TR::Node *node, TR::SymbolReference *ivSymRef)
{
    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference() == ivSymRef))
        return true;
