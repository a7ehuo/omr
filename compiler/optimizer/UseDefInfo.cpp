 ...
 *                                   set to true.
 */
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation *comp, TR::CFG *cfg, TR::Optimizer *optimizer, bool requiresGlobals,
    bool prefersGlobals, bool loadsShouldBeDefs, bool cannotOmitTrivialDefs, bool conversionRegsOnly, bool doCompletion,
    bool callsShouldBeUses)
...
void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs,
    bool conversionRegsOnly)
{
    OMR::Logger *log = comp()->log();
    LexicalTimer tlex("useDefInfo", comp()->phaseTimer());

    TR_UseDefInfo::AuxiliaryData aux(comp()->getSymRefCount(), comp()->getNodeCount(),
        comp()->trMemory()->heapMemoryRegion(), comp()->allocator("UseDefAux"));
    ...
    if (_hasLoadsAsDefs && !cannotOmitTrivialDefs && (comp()->getMethodHotness() < warm)) {
        for (int32_t j = 0; j < numSymRefs; j++) {
            if (symRefTab->getSymRef(j) && symRefTab->getSymRef(j)->getSymbol()->isAutoOrParm()) {
                aux._onceReadSymbolsIndices[j].GrowTo(
                    comp()->getNodeCount()); /* no effect for sparse bit vectors, except to make non-null */
            }
        }
    }

    if (_hasLoadsAsDefs && !cannotOmitTrivialDefs) {
        for (int32_t j = 0; j < numSymRefs; j++) {
            if (symRefTab->getSymRef(j) && symRefTab->getSymRef(j)->getSymbol()->isAutoOrParm()) {
    ...
    if (requiresGlobals || prefersGlobals) {
        ...
        if (requiresGlobals && !canBuild) {
            ...
            logprints(trace(), log,
                "Use/Def info: cannot build global use/def info as it failed in indexSymbolsAndNodes\n");
            return;
        }
    }
    ...
    if (!canBuild) {
        ...
        logprints(trace(), log,
            "Use/Def info: cannot build global nor local use/def info as it failed in indexSymbolsAndNodes\n");
        return;
    }
    ...
    if (trace()) {
        log->printf("Has loads as defs              = %d\n", _hasLoadsAsDefs);
        log->printf("Number of symbols              = %d\n", _numSymbols);
        log->printf("Number of memory symbols       = %d\n", _numMemorySymbols);
        log->printf("Number of statics and fields   = %d\n", _numStaticsAndFields);
        log->printf("Total nodes for use/def info   = %d\n", getTotalNodes());
        log->printf("   Number of defOnly nodes     = %d\n", _numDefOnlyNodes);
        log->printf("   Number of defUse nodes      = %d\n", _numDefUseNodes);
        log->printf("   Number of useOnly nodes     = %d\n", _numUseOnlyNodes);
        log->printf("Total nodes for reaching defs  = %d\n", getExpandedTotalNodes());
        log->printf("   Number of defOnly nodes     = %d\n", _numExpandedDefOnlyNodes);
        log->printf("   Number of defUse nodes      = %d\n", _numExpandedDefUseNodes);
        log->printf("   Number of useOnly nodes     = %d\n", _numExpandedUseOnlyNodes);
        log->printf("   Number of defs on entry     = %d\n", _numDefsOnEntry);
    }
    ...
    logprints(trace(), log, "completed initialization of use/def info\n\n");
...
bool TR_UseDefInfo::performAnalysis(AuxiliaryData &aux)
{
    ...
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "started reaching definition analysis for use/def\n\n");
    ...
    logprints(trace(), log, "completed reaching definition analysis for use/def\n\n");
    return true;
}

bool TR_UseDefInfo::_runReachingDefinitions(TR_ReachingDefinitions &reachingDefinitions, AuxiliaryData &aux)
{
    // stack memory is prohibited in UseDefInfo, but here we have to use it as reachingDefinitions use stack memory for
    // _blockAnalysisInfo
    ...
    if (!succeeded) {
        invalidateUseDefInfo();
        logprints(trace(), comp()->log(), "Method too complex to perform reaching defs, use/def info not built\n");
    } else {
...
bool TR_UseDefInfo::isLoadAddrUse(TR::Node *node) { return node->getOpCodeValue() == TR::loadaddr; }
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        if (symRef->getSymbol()->isAutoOrParm()) {
            if (!aux._onceReadSymbolsIndices[symRefNum].IsNull()) {
                ...
                logprintf(trace(), log, "SETTING node %p symRefNum %d\n", node, symRefNum);
            }

            if (aux._neverWrittenSymbols.get(symRefNum)) {
                aux._neverWrittenSymbols.reset(symRefNum);

                logprintf(trace(), log, "Resetting write bit %d at node %p\n", symRefNum, node);
                ...
                if (!aux._onceWrittenSymbolsIndices[symRefNum].IsNull()) {
                    ...
                    logprintf(trace(), log, "Sym ref %d written once at node %p\n", symRefNum, treeTop->getNode());
                }
            } else if (!aux._onceWrittenSymbolsIndices[symRefNum].IsNull()) {
                aux._onceWrittenSymbolsIndices[symRefNum].ClearToNull();
            }
        }
    } else if (node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node)) {
        TR::SymbolReference *symRef = node->getSymbolReference();
        int32_t symRefNum = symRef->getReferenceNumber();
        if (symRef->getSymbol()->isAutoOrParm()) {
            if (aux._neverReadSymbols.get(symRefNum)) {
                ...
                logprintf(trace(), log, "Resetting read bit %d at node %p\n", symRefNum, node);
                ...
                if ((prevLoadNode->getByteCodeIndex() != node->getByteCodeIndex())
                    ...
                    logprintf(trace(), log, "KILLING bit %d at node %p\n", symRefNum, node);
                }
            }
        }
    }
...
{
    if (node->getOpCode().isStore() && node->getSymbol()->isAutoOrParm() && node->storedValueIsIrrelevant())
        return true;

    if (_useDefForRegs && (node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg()))
        return false;
    ...
    if (_hasLoadsAsDefs && symRef->getSymbol()->isAutoOrParm()) {
        if (!aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull()
            && ((node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node))
                || (node->getOpCode().isStoreDirect()
                    && aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].ValueAt(node->getGlobalIndex()))))
            return true;
    } else if (symRef->getSymbol()->isAutoOrParm() && (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))))
        return true;
...
bool TR_UseDefInfo::isValidAutoOrParm(TR::SymbolReference *symRef)
{
    ...
    return (useDefAliases.PopulationCount() == 1 && useOnlyAliases.PopulationCount() == 1);
}
...
void TR_UseDefInfo::findMemorySymbols(TR::Node *node)
{
    ...
    if ((node->getOpCode().isLoadIndirect() || node->getOpCode().isStoreIndirect())
        && node->getSymbolReference()->getSymbol()->isShadow() && _valueNumberInfo
        && _valueNumberInfo->getNext(node->getFirstChild())
            != node->getFirstChild()) // value number is shared with some other node
    {
        ...
        bool found = false;
        for (auto itr = _valueNumbersToMemorySymbolsMap[valueNumber]->begin(),
                  end = _valueNumbersToMemorySymbolsMap[valueNumber]->end();
             itr != end; ++itr) {
        ...
        logprintf(trace(), comp()->log(), "Node %p has memory symbol index %d (%d:%d:%d)\n", node,
            _valueNumbersToMemorySymbolsMap[valueNumber]->front()._localIndex, valueNumber, size, offset);
    }
}
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData &aux)
{
    ...
    logprintf(trace(), log, "Trying to index nodes with _indexFields = %d, _indexStatics = %d\n", _indexFields,
        _indexStatics);
    ...
    if (trace()) {
        log->printf("_neverReferencedSymbols[count = %d]: ", aux._neverReferencedSymbols.elementCount());
        aux._neverReferencedSymbols.print(log, comp());
        log->println();
    }
        ...
        if (symRef) {
            ...
            if (sym) {
                ...
                if (!sym->isTransparent()) {
                    logprintf(trace(), log,
                        "Ignoring Symbol [%p] because it is non-transparent %d or aliased to a non-transparent %d\n",
                ...
                if (!sym->isStatic() && !sym->isMethodMetaData() && !sym->isShadow())
                    continue;

                // Index this symbol
                //
                logprintf(trace(), log, "Symbol [%p] has index %d\n", sym, _numSymbols);
    ...
    while (refIter.hasMoreElements()) {
        ...
        if (trace()) {
            log->printf("Non Trivial symbol Ref [%p:%d] isTrivial=%d isParm=%d _neverWritten=%d", symRef,
                symRef->getReferenceNumber(), isTrivialUseDefSymRef(symRef, aux), symRef->getSymbol()->isParm(),
        ...
        if (symRef
            && (!isTrivialUseDefSymRef(symRef, aux)
                && ((symRef->getSymbol()->isParm() && !aux._neverWrittenSymbols.get(symRef->getReferenceNumber()))
                    || (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull()
                        && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull())))) {
            sym = symRef->getSymbol();
            logprintf(trace(), log, "   sym=%p localIndex=%d \n", sym, sym->getLocalIndex());
            ...
            if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX) {
                ...
                // Index this symbol
                //
                logprintf(trace(), log, "Non Trivial symbol [%p] has index %d\n", sym, _numSymbols);
    ...
    while (refIter.hasMoreElements()) {
        ...
        if (symRef
            ...
            if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX) {
                ...
                // Index this symbol
                //
                logprintf(trace(), log, "Trivial symbol [%p] has index %d\n", sym, _numSymbols);
                sym->setLocalIndex(_numSymbols++);
            }
        }
    }
    ...
    // A cleared symRefToLocalIndexMap is required for each call to indexSymbolsAndNodes - so we create it as a local
    // variable here, and pass it down to findUseDefNodes, as findUseDefNodes is recursive.
    ...
    int32_t maxDefByUseNodes
        = ((comp()->getMethodHotness() >= hot)) ? MAX_DEF_BY_USE_NODES_FOR_HIGH_OPT : MAX_DEF_BY_USE_NODES_FOR_LOW_OPT;
    ...
    if (trace()) {
        log->printf("      expanded nodes          = %d\n", getExpandedTotalNodes());
        log->printf("      def/use combinations    = %d\n",
            (getNumDefNodes() - getNumIrrelevantStores()) * getNumUseNodes());
        log->printf("      irrelevant stores       = %d\n", getNumIrrelevantStores());
        log->printf("      expanded use nodes      = %d\n", getNumExpandedUseNodes());
        log->printf("      expanded def nodes      = %d\n", getNumExpandedDefNodes());
        log->printf("      local index level  = %d\n", _numExpandedDefUseNodes);
        log->printf("      use/def index level     = %d\n", _numDefOnlyNodes);
    }
    return true;
}

bool TR_UseDefInfo::skipAnalyzingForCompileTime(TR::Node *node, TR::Block *block, TR::Compilation *comp,
    AuxiliaryData &aux)
{
...
bool TR_UseDefInfo::findUseDefNodes(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
    AuxiliaryData &aux, TR::deque<uint32_t, TR::Region &> &symRefToLocalIndexMap, bool considerImplicitStores)
{
    vcount_t visitCount = comp()->getVisitCount();
    if (visitCount == node->getVisitCount())
        return true;
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        bool shouldConsiderLoadAddrChildrenImplicitStores
            = parentCouldHaveImplicitStoreChildren && childIndexIndicatesImplicitStore(node, i);
        if (!findUseDefNodes(block, node->getChild(i), node, treeTop, aux, symRefToLocalIndexMap,
                shouldConsiderLoadAddrChildrenImplicitStores))
            return false;
    }
    ...
    if (_useDefForRegs && (opCode.isLoadReg() || opCode.isStoreReg())) {
        symRef = NULL;
        num_aliases = 1;
    ...
    if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
        if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
        ...
        } else if (isLoadAddrUse(node) || opCode.isLoadVar() || (_useDefForRegs && opCode.isLoadReg())) {
            if (!skipAnalyzingForCompileTime(node, block, comp(), aux))
            ...
            //    node->setUseDefIndex(0);
            //    }
        } else if (opCode.isStore() || (_useDefForRegs && opCode.isStoreReg())
            ...
            if (isTrivialUseDefNode(node, aux)) {
                if (symRef
                    ...
                    if (aux._onceReadSymbols[symRef->getReferenceNumber()] == NULL)
                        aux._onceReadSymbols[symRef->getReferenceNumber()]
                            = new (aux._region) TR_BitVector(aux._region);
                ...
                if (symRef
                    ...
                    if (aux._onceWrittenSymbols[symRef->getReferenceNumber()] == NULL)
                        aux._onceWrittenSymbols[symRef->getReferenceNumber()]
                            = new (aux._region) TR_BitVector(aux._region);
    ...
    } else if (_indexFields && node->isGCSafePointWithSymRef() && comp()->getOptions()->realTimeGC()) {
        localIndex = _numExpandedDefOnlyNodes;
        _numExpandedDefOnlyNodes
            += TR::NumAllTypes + 1; // == arraylet shadows + read barrier....encode this where?  SymRefTab probably
        useDefIndex = _numDefOnlyNodes++;
    }
    // else
    //    {
...
bool TR_UseDefInfo::assignAdjustedNodeIndex(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
    AuxiliaryData &aux, bool considerImplicitStores)
{
    vcount_t visitCount = comp()->getVisitCount();
    if (visitCount == node->getVisitCount())
        return true;
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        bool shouldConsiderLoadAddrChildrenImplicitStores
            = parentCouldHaveImplicitStoreChildren && childIndexIndicatesImplicitStore(node, i);
        if (!assignAdjustedNodeIndex(block, node->getChild(i), node, treeTop, aux,
                shouldConsiderLoadAddrChildrenImplicitStores))
            return false;
    }
...
bool TR_UseDefInfo::childIndexIndicatesImplicitStore(TR::Node *node, int32_t childIndex) { return false; }

void TR_UseDefInfo::insertData(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
    AuxiliaryData &aux, TR::SparseBitVector &expandedLIndexes, bool considerImplicitStores)
{
    vcount_t visitCount = comp()->getVisitCount();
    if (visitCount == node->getVisitCount())
        return;
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        bool shouldConsiderLoadAddrChildrenImplicitStores
            = parentCouldHaveImplicitStoreChildren && childIndexIndicatesImplicitStore(node, i);
        insertData(block, node->getChild(i), node, treeTop, aux, expandedLIndexes,
            shouldConsiderLoadAddrChildrenImplicitStores);
    }
    ...
    if (trace()) {
        if (!definesMultipleSymbols)
            log->printf("Node : %p   opCode = %s useDefIndex = %d localIndex = %d definesMultipleSymbols=%d "
                        "isTrivialUseDefNode=%d adjustArray=%d \n",
                node, opCode.getName(), node->getUseDefIndex(), node->getLocalIndex(), definesMultipleSymbols,
                isTrivialUseDefNode(node, aux), adjustArray);
        else
            log->printf("Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d definesMultipleSymbols=%d "
                        "isTrivialUseDefNode=%d adjustArray=%d \n",
                node, opCode.getName(), node->getUseDefIndex(), node->getLocalIndex(),
                node->getLocalIndex() + num_aliases - 1, definesMultipleSymbols, isTrivialUseDefNode(node, aux),
                adjustArray);
    }

    nodeIndex = node->getLocalIndex();

    if (definesMultipleSymbols && !isTrivialUseDefNode(node, aux) && adjustArray)

    {
        ...
        for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
            ...
            logprintf(trace(), log, "    symbol (u/d index=%d) is defined by node with localIndex %d \n", j, k);
        }
...
#endif
    else if (definesSingleSymbol && !isTrivialUseDefNode(node, aux) && adjustArray) {
...
bool isSymRefFromInlinedMethod(TR::Compilation *comp, TR::ResolvedMethodSymbol *currentMethod,
    TR::SymbolReference *symRef)
{
    while (true) {
...
void TR_UseDefInfo::processReachingDefinition(void *vblockInfo, AuxiliaryData &aux) { buildUseDefs(vblockInfo, aux); }
...
void TR_UseDefInfo::buildUseDefs(void *vblockInfo, AuxiliaryData &aux)
{
    ...
    if (_indexStatics && blockInfo) {
        ...
        if (trace()) {
            log->printf("Found exit block_%d", _cfg->getEnd()->getNumber());
            analysisInfo->print(log, comp());
            log->printf(" Use index %d\n", getLastUseIndex() - getFirstUseIndex());
            log->println();
        }
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (i >= _numDefsOnEntry) {
                ...
                if (!(getNode(i)->getOpCode().hasSymbolReference()
                        && getNode(i)->getSymbolReference()->getSymbol()->isStatic())
                    && !externalAutoParm)
                    continue;
        ...
        if (trace()) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (defIndex >= getFirstUseIndex())
                    log->printf("      Single defining load #%d[%p]", defIndex, getNode(defIndex));
                else
                    log->printf("      Def #%d[%p]", defIndex, getNode(defIndex));
                if (defIndex < getNumDefsOnEntry())
                    log->prints(" (from method entry) ");
                log->println();
            }
        }
    }
...
void TR_UseDefInfo::dereferenceDefs(int32_t useIndex, TR_UseDefInfo::BitVector &nodesLookedAt,
    TR_UseDefInfo::BitVector &loadDefs)
{
    OMR::Logger *log = comp()->log();
    if (trace()) {
    ...
    if (!loadDefs.IsZero()) {
        ...
        useDefInfo |= loadDefs;
        if (trace()) {
            log->printf("      Changing use index %d to have defining loads : \n", useIndex + getFirstUseIndex());
            (*comp()) << loadDefs;
            log->println();
        }
    }
    ...
    if (trace()) {
        log->printf("New defs for use index %d : ", useIndex + getFirstUseIndex());
        (*comp()) << _useDefInfo[useIndex];
        log->println();
    }
}
...
//
int32_t TR_UseDefInfo::setSingleDefiningLoad(int32_t useIndex, TR_UseDefInfo::BitVector &nodesLookedAt,
    TR_UseDefInfo::BitVector &loadDefs)
{
    OMR::Logger *log = comp()->log();
    TR_UseDefInfo::BitVector &useDefInfo = _useDefInfo[useIndex];
    ...
    if (trace()) {
        log->printf("   Checking use index %d for single defining load : ", useIndex + getFirstUseIndex());
        (*comp()) << useDefInfo;
        log->println();
    }
    ...
    //
    if ((defIndex < getFirstUseIndex()) || !getNode(defIndex)->getOpCode().isLoadVar())
        return -2;
    ...
    if (0 && !cursor.Valid()) {
        if (nodesLookedAt.ValueAt(defIndex - getFirstUseIndex())) {
            logprintf(trace(), log, "      Use index %d has circular defining loads\n", useIndex + getFirstUseIndex());
            return -1;
        }
        logprintf(trace(), log, "      Use index %d has single defining load %d\n", useIndex + getFirstUseIndex(),
            defIndex);
    ...
    for (; cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!nodesLookedAt.ValueAt(defIndex)) {
            ...
            if (singleDef == -2) {
                ...
                logprintf(trace(), log, "      Use index %d has defining load %d\n", useIndex + getFirstUseIndex(),
                    defIndex + getFirstUseIndex());
    ...
    if (0 && trace()) {
        if (commonDef >= 0)
            log->printf("      Use index %d has single defining load %d\n", useIndex + getFirstUseIndex(), commonDef);
        else
            log->printf("      Use index %d has circular defining loads\n", useIndex + getFirstUseIndex());
    }
 ...
 * in the same block
 */
void TR_UseDefInfo::dereferenceDef(TR_UseDefInfo::BitVector &useDefInfo, int32_t defIndex,
    TR_UseDefInfo::BitVector &nodesLookedAt)
{
    ...
    while (!defIndices.empty()) {
        ...
        if (trace()) {
            log->printf("   De-referencing use index %d : ", defIndex);
            (*comp()) << useDefInfo;
            log->println();
        }

        {
            logprintf(trace(), log, "      Resetting def index %d\n", defIndex);
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (myDef < getFirstUseIndex() || getNode(myDef)->getSymbolReference()->getSymbol()->isMethod()) {
                    logprintf(trace(), log, "      Setting def index %d\n", myDef);
                    useDefInfo[myDef] = true;
                } else if (!nodesLookedAt.ValueAt(myDef - getFirstUseIndex())) {
                    logprintf(trace(), log, "      Adding def index %d\n", myDef);
...
void TR_UseDefInfo::buildUseDefs(TR::Node *node, void *vanalysisInfo, TR_BitVector &nodesToBeDereferenced,
    TR::Node *parent, AuxiliaryData &aux)
{
    ...
    TR::Method *method = comp()->getMethodSymbol()->getMethod();
    uint32_t nodeIndex = node->getUseDefIndex();
    if (node->getOpCode().hasSymbolReference() && isTrivialUseDefNode(node, aux)) {
        TR::SymbolReference *symRef = node->getSymbolReference();
        TR::Symbol *sym = symRef->getSymbol();
        bool defsKnownTrivially = false;
        if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
            int32_t realIndex = nodeIndex - getFirstUseIndex();
            logprintf(trace(), log, "For node %p index = %d and first use index = %d\n", node, nodeIndex,
                getFirstUseIndex());
            if (aux._neverWrittenSymbols.get(node->getSymbolReference()->getReferenceNumber())) {
                ...
                logprintf(trace(), log, "Reached here (entry) for use node %p\n", node);
            } else if (!aux._onceReadSymbolsIndices[node->getSymbolReference()->getReferenceNumber()].IsNull()) {
                logprintf(trace(), log, "Use node %p is of a symbol read only once\n", node);

                TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex = %d\n", realIndex);
                if (aux._onceReadSymbols[node->getSymbolReference()->getReferenceNumber()])
                    _useDefInfo[realIndex]
                        |= CS2_TR_BitVector(*(aux._onceReadSymbols[node->getSymbolReference()->getReferenceNumber()]));
                defsKnownTrivially = true;

            } else if (!aux._onceWrittenSymbolsIndices[node->getSymbolReference()->getReferenceNumber()].IsNull()) {
                logprintf(trace(), log, "Use node %p is of a symbol write only once\n", node);

                TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex = %d\n", realIndex);
                if (aux._onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()])
                    _useDefInfo[realIndex] |= CS2_TR_BitVector(
                        *(aux._onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()]));
                defsKnownTrivially = true;
            } else {
    ...
    if (isExpandedUseIndex(expandedNodeIndex)
        ...
        if (!aux._defsForSymbol[symIndex]->isEmpty()) {
            ...
            if (trace()) {
                log->printf("defs for symbol %d node:%p \n", symIndex, node);
                aux._defsForSymbol[symIndex]->print(log, comp());
                log->println();
            }
            ...
            *defs = *(aux._defsForSymbol[symIndex]);
            ...
            if (memSymIndex != -1 && !aux._defsForSymbol[memSymIndex]->isEmpty()) {
                if (trace()) {
                    log->printf("defs for memory symbol %d \n", memSymIndex);
                    aux._defsForSymbol[memSymIndex]->print(log, comp());
                    log->println();
                }
            ...
            while (cursor.hasMoreElements()) {
                ...
                if (i >= _numDefsOnEntry) {
                    ...
                    logprintf(trace(), log, "reached by expanded index %d [0x%p]\n", i, defNode);

                    auto j = defNode->getUseDefIndex();
                    if (isUseIndex(j))
                        hasUseAsDef = true;

                    if (memSymIndex == -1 || getMemorySymbolIndex(defNode) != memSymIndex
    ...
    if (symIndex == NULL_USEDEF_SYMBOL_INDEX || node->getOpCode().isCall() || node->getOpCode().isFence()
        ...
        //
        numDefNodes = 1;
        if (!aux._defsForSymbol[symIndex]->isEmpty() && (!sym || (!sym->isShadow() && !sym->isMethod()))) {
            {
...
TR::Node *TR_UseDefInfo::getSingleDefiningLoad(TR::Node *node)
{
    ...
    if (info.PopulationCount() == 1) {
        ...
        if ((firstDef >= getFirstUseIndex()) && (n = getNode(firstDef)) && (n->getUseDefIndex() > 0)
            && (n->getOpCode().isLoadVar()) && (n->getOpCode().hasSymbolReference() && n->getSymbol()->isTransparent()))
            return n;
    }
    return NULL;
}
...
const TR_UseDefInfo::BitVector &TR_UseDefInfo::getUseDef_ref_body(int32_t useIndex, TR_BitVector *visitedDefs,
    TR_UseDefInfo::BitVector *defs)
{
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (firstDef < 0) {
                ...
                if ((firstDef < getFirstUseIndex())
                    || (getNode(useIndex) && getNode(useIndex)->getSymbolReference()
...
void TR_UseDefInfo::buildDefUseInfo(bool loadAsDef)
{
    ...
    if ((_defUseInfo.size() > 0) && ((_loadDefUseInfo.size() > 0) || !loadAsDef))
        return;
    ...
    for (int32_t i = getFirstUseIndex(); i <= getLastUseIndex(); i++) {
        ...
        if (!defs.IsZero()) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                int32_t defIndex = cursor;
                TR_ASSERT((defIndex < getNumDefNodes()),
                    "USEDEF: found def which is not store or call: useIndex = %d, defIndex = %d", i, defIndex);
        ...
        if (loadAsDef) {
            ...
            if (!loadDefs.IsZero()) {
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    int32_t defIndex = cursor;
                    // TR_ASSERT((defIndex < getNumDefNodes()), "USEDEF: found def which is not store or call %d",
                    // defIndex);
