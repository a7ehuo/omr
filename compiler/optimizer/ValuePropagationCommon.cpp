...
void OMR::ValuePropagation::initialize()
{
    ...
    _constraintsHashTable = (ConstraintsHashTableEntry **)trMemory()->allocateStackMemory(
        VP_HASH_TABLE_SIZE * sizeof(ConstraintsHashTableEntry *));
    ...
    while (curTree) {
        if (curTree->getNode())
            collectArraylengthNodes(curTree->getNode(), visitCount, &_arraylengthNodes);
    ...
    if (_isGlobalPropagation) {
        ...
        _globalConstraintsHashTable = (GlobalConstraint **)trMemory()->allocateStackMemory(
            globalConstraintsHashTableSize * sizeof(GlobalConstraint *));
        memset(_globalConstraintsHashTable, 0, globalConstraintsHashTableSize * sizeof(GlobalConstraint *));

        _edgeConstraintsHashTable
            = (EdgeConstraints **)trMemory()->allocateStackMemory(VP_HASH_TABLE_SIZE * sizeof(EdgeConstraints *));
        memset(_edgeConstraintsHashTable, 0, VP_HASH_TABLE_SIZE * sizeof(EdgeConstraints *));

        _loopDefsHashTable = (LoopDefsHashTableEntry **)trMemory()->allocateStackMemory(
            VP_HASH_TABLE_SIZE * sizeof(LoopDefsHashTableEntry *));
    ...
    if (_isGlobalPropagation) {
        _definedOnAllPaths
            = new (trStackMemory()) DefinedOnAllPathsMap(std::less<TR::CFGEdge *>(), trMemory()->currentStackRegion());
        _callNodeToGuardNodes
            = new (trStackMemory()) CallNodeToGuardNodesMap(std::less<TR::Node *>(), trMemory()->currentStackRegion());
    } else {
    ...
    if (0 && !disablePREX && !disablePREXinVP && comp()->getMethodHotness() >= warm && comp()->couldBeRecompiled()
        && !comp()->getRecompilationInfo()->isProfilingCompilation() && !comp()->getOption(TR_DisableCHOpts)) {
        enablePreexistence();
    }
    ...
    if (((comp()->getMethodHotness() > warm) || !comp()->isOptServer())
        ...
        if (comp()->getFlowGraph()->getStructure()) {
            ...
            // The bitVector is growable /bc when we devirtaulize calls we add symbol references for the call and the
            // parameters.
            _seenDefinedSymbolReferences
                = new (trStackMemory()) TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
...
OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::copyStoreRelationships(StoreRelationship *first)
{
    ...
    for (StoreRelationship *rel = first; rel; rel = rel->getNext()) {
        StoreRelationship *newRel
            = createStoreRelationship(rel->symbol, copyRelationships(rel->relationships.getFirst()));
        list.append(newRel);
    }
    return list.getFirst();
}

OMR::ValuePropagation::ValueConstraint *OMR::ValuePropagation::createValueConstraint(int32_t valueNumber,
    Relationship *relationships, StoreRelationship *storeRelationships)
{
    ValueConstraint *vc;
...
int32_t OMR::ValuePropagation::getValueNumber(TR::Node *node)
{
    ...
    if (_isGlobalPropagation)
        ...
        if (node->getOpCode().isStore()) {
        ...
        } else if (node->getOpCode().isLoadConst() && node->canGet64bitIntegralValue()
            ...
            if (_constNodeInfo.Locate(value, index)) {
                ...
                TR::Node *result = NULL;
                for (auto nodeCursor = nodeList->begin(); (result == NULL) && (nodeCursor != nodeList->end());
                     ++nodeCursor) {
                ...
                TR::list<TR::Node *> *nodeList = new (comp()->trHeapMemory())
                    TR::list<TR::Node *>(getTypedAllocator<TR::Node *>(comp()->allocator()));
                nodeList->push_back(node);
...
TR::VPConstraint *OMR::ValuePropagation::getConstraint(TR::Node *node, bool &isGlobal, TR::Node *relative)
{
    ...
    if (rel) {
        if (trace()) {
            log->printf("   %s [%p] has existing constraint:", node->getOpCode().getName(), node);
    ...
    if (rel) {
        if (trace()) {
            log->printf("   %s [%p] has existing global constraint:", node->getOpCode().getName(), node);
            rel->print(log, this, valueNumber, 1);
        }
        isGlobal = true;
        constraint = rel->constraint;
...
void OMR::ValuePropagation::processTrees(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
    ...
    if (_enableVersionBlocks && lastTimeThrough() && (startTree->getNode()->getOpCodeValue() == TR::BBStart)) {
    ...
    for (treeTop = startTree; ((treeTop != endTree) && (treeTop != _curBlock->getExit()));
         treeTop = _curTree->getNextTreeTop()) {
        _curTree = treeTop;
        TR::Node *treeTopNode = treeTop->getNode();
        logprintf(trace(), comp()->log(), "Processing ttNode n%in %s\n", treeTopNode->getGlobalIndex(),
...
TR::Node *generateLenForArrayCopy(TR::Compilation *comp, int32_t elementSize, TR::Node *stride, TR::Node *srcObjNode,
    TR::Node *copyLenNode, TR::Node *n)
{
    ...
    if (elementSize == 1) {
        ...
        if (TR::Compiler->om.isOffHeapAllocationEnabled())
            len = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp, copyLenNode, stride,
                elementSize, true);
        else
...
bool OMR::ValuePropagation::canRunTransformToArrayCopy()
{
    ...
    if ((comp()->getMethodHotness() >= hot) && !_isGlobalPropagation && !getLastRun())
        return false;
...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node)
{
    ...
    // removed.
    //
    if ((srcObject && srcObject->isNullObject()) || (dstObject && dstObject->isNullObject()) || srcOffHigh < 0
        || dstOffHigh < 0 || copyLenHigh < 0
        || (srcObject && srcObject->getClassType() && srcObject->getClassType()->asFixedClass()
            && (srcObject->getClassType()->asFixedClass()->isArray() == TR_no))
        || (dstObject && dstObject->getClassType() && dstObject->getClassType()->asFixedClass()
            && (dstObject->getClassType()->asFixedClass()->isArray() == TR_no))) {
...
#ifdef J9_PROJECT_SPECIFIC
    bool isStringCompressedArrayCopy = recognizedMethod == TR::java_lang_String_compressedArrayCopy_BIBII
    ...
    bool isStringDecompressedArrayCopy = recognizedMethod == TR::java_lang_String_decompressedArrayCopy_BIBII
    ...
    //
    if ((primitiveTransform || referenceTransform) && !comp()->getOption(TR_DisableArrayCopyOpts)
        && !node->isDontTransformArrayCopyCall() && !comp()->getOption(TR_DisableInliningOfNatives)
        && !(srcObject && srcObject->isNullObject()) && !(dstObject && dstObject->isNullObject()) && srcOffHigh >= 0
        && dstOffHigh >= 0 && copyLenHigh >= 0) {
        ...
        if (comp()->generateArraylets() && !comp()->getOption(TR_DisableMultiLeafArrayCopy)) {
            ...
            if (caller && strncmp(caller->nameChars(), sig, strlen(sig)) == 0) {
                logprintf(trace(), log, "Detected real-time same leaf arraycopy: %p from java helper\n", node);
                ...
                if (primitiveArray2) {
                    if (srcObject->getClassType() == dstObject->getClassType()
                        || (srcObject->getClassType() && dstObject->getClassType()
                            ...
                            && srcObject->getClassType()->asResolvedClass()->getClass()
                                == dstObject->getClassType()->asResolvedClass()->getClass())) {
                        needArrayCheck = false;
                    } else {
                ...
                if (referenceArray2) {
                    if (dstObject && dstObject->getClass()) {
                        ...
                        if (sig && sig[0] == '[') {
                            ...
                            //
                            if (len == 19 && dstObject->isFixedClass() && !strncmp(sig, "[Ljava/lang/Object;", 19)) {
                                needArrayStoreCheck = false;
                            }
                            ...
                            //
                                ...
                                isInstance = fe()->isInstanceOf(srcObject->getClass(), dstObject->getClass(),
                                    srcObject->isFixedClass(), dstObject->isFixedClass());
        ...
        if (primitiveArray1 || primitiveArray2) {
            type = primitiveArray1 ? srcObject->getClassType()->getPrimitiveArrayDataType()
        ...
        if (comp()->getOptions()->realTimeGC() && comp()->requiresSpineChecks()
        ...
        if (transformTheCall && comp()->generateArraylets()) {
            if (!((primitiveArray1 || primitiveArray2)
                ...
                if ((srcOffLow < 0) || (srcOffHigh > TR::getMaxSigned<TR::Int32>() - copyLenHigh))
                    sameLeafSrc = false;
                else if (fe()->getArrayletLeafIndex(srcOffLow, elementSize)
                    == fe()->getArrayletLeafIndex(srcOffHigh + copyLenHigh, elementSize))
                    sameLeafSrc = true;

                if ((dstOffLow < 0) || (dstOffHigh > TR::getMaxSigned<TR::Int32>() - copyLenHigh))
                    sameLeafDst = false;
                else if (fe()->getArrayletLeafIndex(dstOffLow, elementSize)
                    == fe()->getArrayletLeafIndex(dstOffHigh + copyLenHigh, elementSize))
                    sameLeafDst = true;

                if (!sameLeafSrc) {
                    if ((copyLenNode->getOpCode().isLoadConst() && srcOffNode->getOpCode().isLoadConst())
                ...
                if (!sameLeafDst) {
                    if ((copyLenNode->getOpCode().isLoadConst() && dstOffNode->getOpCode().isLoadConst())
                        && !isRecognizedMultiLeafArrayCopy) {
                        if (!comp()->getOption(TR_DisableMultiLeafArrayCopy))
                            isMultiLeafArrayCopy = true;
                        else
                    ...
                    if (isSrcArrayNullRestricted == TR_yes) {
                        ...
                        if (isValueTypeArrayFlatteningEnabled) {
                            ...
                            if ((isArrayElementFlattened(dstObject) == TR_yes)
                                ...
                                logprintf(trace(), log, "%s: n%dn %p elementSize %d\n", __FUNCTION__,
                    ...
                    if (isValueTypeArrayFlatteningEnabled) {
                        if (isSrcArrayNullRestricted == TR_yes) {
                            if (isArrayElementFlattened(srcObject) == TR_yes) {
                                logprintf(trace(), log,
                                    "%s: n%dn %p dst array is identity type and source array is flattened VT array\n",
                            ...
                            // The source might or might not be a null-restricted array. If it is null-restricted array
                            // that is flattened, the arraycopy instruction cannot handle copying from flattened array
                            // into non-flattened array since the destination array is identity type. Need to add
                            // runtime check here
                            doRuntimeNullRestrictedTest = true;
                            // We already know the destination array is identity type. No need to insert runtime test
                            // here
    ...
    slowBlock---> n39n      BBStart <block_-1>
                  n41n      treetop
                  n42n        call  java/lang/System.arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V [#428  final
    native static Method] [flags 0x20500 0x0 ] (dontTransformArrayCopyCall ) n49n          aload  <temp slot 3>[#429
    Auto] n51n          iconst 0 n53n          aload  <temp slot 5>[#431  Auto] n55n          iconst 0 n57n iload  <temp
    slot 7>[#433  Auto] n40n      BBEnd </block_-1>
               */
            ...
            TR::Node *newCallNode = node->duplicateTree(
                false); // No need to duplicate the children of the call since they will be replaced next with temp load
            // It is important to set setDontTransformArrayCopyCall on the new call node. Otherwise, the new call node
            // could get transformed again by VP
            ...
            for (int32_t i = 0; i < oldCallNode->getNumChildren(); ++i) {
                ...
                if (child->getOpCode().isLoadConst()) {
                    ...
                    logprintf(trace(), log,
                        "Created child n%dn %p #%d for old child n%dn %p of the original call node\n",
                ...
                logprintf(trace(), log, "Created child n%dn %p for new call node n%dn %p\n", value->getGlobalIndex(),
            ...
            // nextTT is required so that we can jump from the slow block to the block that contains the treetop after
            // System.arraycopy
            ...
            _needRuntimeTestNullRestrictedArrayCopy.add(
        ...
        bool canSkipAllChecksOnArrayCopy
            = comp()->getOwningMethodSymbol(node->getOwningMethod())->safeToSkipChecksOnArrayCopies()
            || isRecognizedMultiLeafArrayCopy || isStringCompressedArrayCopy || isStringDecompressedArrayCopy;
        ...
        if (canSkipAllChecksOnArrayCopy && (isStringCompressedArrayCopy || isStringDecompressedArrayCopy)) {
        ...
        if (!canSkipAllChecksOnArrayCopy) {
            ...
            //
#ifdef J9_PROJECT_SPECIFIC
            if (srcObjNode->getOpCode().hasSymbolReference() && srcObjNode->getSymbolReference()->getSymbol()
                && srcObjNode->getSymbolReference()->getSymbol()->getRecognizedField()
                    == TR::Symbol::Java_lang_String_value) {
                isSrcPossiblyNull = false;
                isStringFwdArrayCopy = true;
            }

            if (dstObjNode->getOpCode().hasSymbolReference() && dstObjNode->getSymbolReference()->getSymbol()
                && dstObjNode->getSymbolReference()->getSymbol()->getRecognizedField()
                    == TR::Symbol::Java_lang_String_value) {
            ...
            if (isSrcPossiblyNull) {
                ...
                prevTree = TR::TreeTop::create(comp(), prevTree,
                    TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, srcArrayLength,
            ...
            if (isDstPossiblyNull) {
                ...
                prevTree = TR::TreeTop::create(comp(), prevTree,
                    TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, dstArrayLength,
                        comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(comp()->getMethodSymbol())));
            }
            ...
            if (needArrayCheck) {
                tempNode = TR::Node::createWithSymRef(TR::ArrayCHK, 2, 2, srcObjNode, dstObjNode,
            ...
            bool needSrcBndChk = !srcArrayInfo || srcOffHigh > TR::getMaxSigned<TR::Int32>() - copyLenHigh
        ...
        //
        if (zeroLengthCopy
            && performTransformation(comp(), "%sRemoving zero length call %s [%p]\n", OPT_DETAILS,
                node->getOpCode().getName(), node)) {
        ...
        if (comp()->requiresSpineChecks()) {
            ...
            _arrayCopySpineCheck.add(new (trStackMemory()) TR_ArrayCopySpineCheck(arraycopyTree,
                node->getSymbolReference(), srcObjNode, srcOffNode, dstObjNode, dstOffNode, copyLenNode));
        }

        if (isMultiLeafArrayCopy) {
            uint8_t flag = 0;

            _needMultiLeafArrayCopy.add(new (trStackMemory()) TR_RealTimeArrayCopy(arraycopyTree, type, flag));
            ...
            if (((dstOffHigh == 0) || (srcOffLow >= dstOffHigh)
        ...
        if (!comp()->generateArraylets()) {
            ...
            src = generateArrayAddressTree(comp(), node, srcOffHigh, srcOffNode, srcObjNode, elementSize, stride,
                hdrSize);
            ...
            dst = generateArrayAddressTree(comp(), node, dstOffHigh, dstOffNode, dstObjNode, elementSize, stride,
                hdrSize);
        } else {
            // aiadd
            //    aloadi
            ...
            TR::Node *strideShiftNode
                = strideShift ? TR::Node::create(node, TR::iconst, 0, (int32_t)strideShift) : NULL;
            ...
            src = generateArrayletAddressTree(comp(), node, type, srcOff, srcObjNode, spineShiftNode, shiftNode,
                strideShiftNode, hdrSize);

            // -------------------------------------------------------------------
            // Calculate destination address node
            // -------------------------------------------------------------------

            dst = generateArrayletAddressTree(comp(), node, type, dstOff, dstObjNode, spineShiftNode, shiftNode,
                strideShiftNode, hdrSize);

            if (comp()->useCompressedPointers()) {
                TR_ASSERT((src->getFirstChild()->getOpCodeValue() == TR::aloadi),
                    "didn't find the src arraylet pointer");
                TR_ASSERT((dst->getFirstChild()->getOpCodeValue() == TR::aloadi),
                    "didn't find the dst arraylet pointer");

                prevTree
                    = TR::TreeTop::create(comp(), prevTree, TR::Node::createCompressedRefsAnchor(src->getFirstChild()));
                prevTree
                    = TR::TreeTop::create(comp(), prevTree, TR::Node::createCompressedRefsAnchor(dst->getFirstChild()));
            }
        }
        ...
        logprintf(trace(), log, "%s: n%dn %p elementSize %d stride n%dn %p\n", __FUNCTION__, node->getGlobalIndex(),
...
createExceptionEdgeConstraintsAndReturn:
    ...
    addBlockConstraint(copyLenNode,
        TR::VPIntRange::create(this, 0, static_cast<int32_t>(TR::getMaxSigned<TR::Int32>())));
}

TR::Node *generateArrayAddressTree(TR::Compilation *comp, TR::Node *node, int32_t offHigh, TR::Node *offNode,
    TR::Node *objNode, int32_t elementSize, TR::Node *&stride, TR::Node *hdrSize)
{
    bool is64BitTarget = comp->target().is64Bit() ? true : false;
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        if (offHigh > 0) {
            if (elementSize == 1)
                ...
                array = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp, offNode, stride,
                    elementSize, true);
            } else {
                array = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp, offNode, stride,
                    elementSize, false);
...
#ifdef J9_PROJECT_SPECIFIC
    TR::Node *vftLoad
        = TR::Node::createWithSymRef(TR::aloadi, 1, 1, node, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
    TR::Node *componentTypeLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, vftLoad,
        comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
    TR::Node *romClassLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, componentTypeLoad,
        comp()->getSymRefTab()->findOrCreateClassRomPtrSymbolRef());
    TR::Node *isArrayField = TR::Node::createWithSymRef(TR::iloadi, 1, 1, romClassLoad,
        comp()->getSymRefTab()->findOrCreateClassIsArraySymbolRef());
    TR::Node *andConstNode
        = TR::Node::create(isArrayField, TR::iconst, 0, TR::Compiler->cls.flagValueForPrimitiveTypeCheck(comp()));
...
{
    TR::Node *vftDst
        = TR::Node::createWithSymRef(TR::aloadi, 1, 1, dst, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
    TR::Node *instanceofNode = TR::Node::createWithSymRef(
        TR:: instanceof, 2, 2, src, vftDst,
...
TR::TreeTop *OMR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    bool useFlagsOnOriginalArraycopy, bool isOptimizedReferenceArraycopy)
{
    ...
    logprintf(trace(), comp()->log(), "Created 3-child arraycopy %s from root node %s, type = %s\n",
...
TR::TreeTop *createStoresForArraycopyChildren(TR::Compilation *comp, TR::TreeTop *arrayTree,
...
#ifdef J9_PROJECT_SPECIFIC
void OMR::ValuePropagation::generateArrayTranslateNode(TR::TreeTop *callTree, TR::TreeTop *arrayTranslateTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *srcOffRef,
    TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, TR::SymbolReference *tableRef, bool hasTable)
{
    bool is64BitTarget = comp()->target().is64Bit();
    ...
    TR::Node *srcOffNode
        = srcOffRef ? TR::Node::createLoad(callNode, srcOffRef) : callNode->getChild(childId++)->duplicateTree();
    TR::Node *srcOff = srcOffNode->createLongIfNeeded();
    ...
    TR::Node *dstOffNode
        = dstOffRef ? TR::Node::createLoad(callNode, dstOffRef) : callNode->getChild(childId++)->duplicateTree();
    TR::Node *dstOff = dstOffNode->createLongIfNeeded();
    ...
    TR::Node *arrayTranslateNode
        = TR::Node::create(arrayTranslateTree->getNode()->getFirstChild(), TR::arraytranslate, 6);
    ...
    */
    ...
    if (encode && !isSBCSEncoder && !isEncodeFromLatin1) {
        ...
        {
            bool genSIMD
                = comp()->cg()->getSupportsVectorRegisters() && !comp()->getOption(TR_DisableSIMDArrayTranslate);
        ...
        arrayTranslateNode->setSourceCellIsTermChar(false);
        arrayTranslateNode->setTableBackedByRawStorage(true);
        if (cg()->getSupportsArrayTranslateTROTNoBreak() || cg()->getSupportsArrayTranslateTROT()) { // X or P
    ...
    } else if (isSBCSEncoder || isEncodeFromLatin1) // only z
    {
        ...
        TR::Node *tableNodeAddr
            = tableRef ? TR::Node::createLoad(callNode, tableRef) : callNode->getChild(childId++)->duplicateTree();
    ...
    if (isEncodeUtf16) {
        TR::SymbolReference *transformedCallSymRef
            = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
                TR::sun_nio_cs_UTF16_Encoder_encodeUTF16Big == rm ? "transformedEncodeUTF16Big"
                                                                  : "transformedEncodeUTF16Little",
                "(JJI)I", TR::MethodSymbol::Static);
...
TR::TreeTop *OMR::ValuePropagation::createConverterCallNodeAfterStores(TR::TreeTop *tree, TR::TreeTop *origTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef, TR::SymbolReference *thisRef,
    TR::SymbolReference *tableRef)
{
    ...
    if (root->getOpCode().isCallIndirect()) {
        TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, thisNode,
    ...
    if (lenRef)
        ...
        len = (isISO88591Encoder || isAsciiEncoder) ? root->getChild(childId + 4)->duplicateTree()
                                                    : root->getChild(childId + 2)->duplicateTree();

    (isISO88591Encoder || isAsciiEncoder) ? root->setAndIncChild(childId + 4, len)
                                          : root->setAndIncChild(childId + 2, len);

    if (dstRef)
        dst = TR::Node::createLoad(root, dstRef);
    else
        dst = (isISO88591Encoder || isAsciiEncoder) ? root->getChild(childId + 2)->duplicateTree()
                                                    : root->getChild(childId + 3)->duplicateTree();

    (isISO88591Encoder || isAsciiEncoder) ? root->setAndIncChild(childId + 2, dst)
                                          : root->setAndIncChild(childId + 3, dst);

    if (dstOffRef)
        dstOff = TR::Node::createLoad(root, dstOffRef);
    else
        dstOff = (isISO88591Encoder || isAsciiEncoder) ? root->getChild(childId + 3)->duplicateTree()
                                                       : root->getChild(childId + 4)->duplicateTree();

    (isISO88591Encoder || isAsciiEncoder) ? root->setAndIncChild(childId + 3, dstOff)
                                          : root->setAndIncChild(childId + 4, dstOff);
...
TR::TreeTop *OMR::ValuePropagation::buildSameLeafTest(TR::Node *offset, TR::Node *len, TR::Node *spineShiftNode)
{
    ...
    // check with this :ifNode = TR::Node::createif(TR::ificmpne, TR::Node::create(offset , TR::iconst, 0, 1),
    // TR::Node::create(offset , TR::iconst, 1, 1));
    ifTree->setNode(ifNode);

    return ifTree;
}

TR::Node *generateArrayletAddressTree(TR::Compilation *comp, TR::Node *vcallNode, TR::DataType type, TR::Node *off,
    TR::Node *obj, TR::Node *spineShiftNode, TR::Node *shiftNode, TR::Node *strideShiftNode, TR::Node *hdrSize)
{
    bool is64BitTarget = comp->target().is64Bit() ? true : false;
    ...
    node = TR::Node::createWithSymRef(TR::aloadi, 1, 1, node,
        comp->getSymRefTab()->findOrCreateArrayletShadowSymbolRef(type));
...
void OMR::ValuePropagation::generateRTArrayNodeWithoutFlags(TR_RealTimeArrayCopy *rtArrayCopy,
    TR::TreeTop *dupArraycopyTree, TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, bool primitive)
{
    ...
    srcOff = generateArrayletAddressTree(comp(), node, type, srcOff, srcObj, spineShiftNode, shiftNode, strideShiftNode,
        hdrSize);
    dstOff = generateArrayletAddressTree(comp(), node, type, dstOff, dstObj, spineShiftNode, shiftNode, strideShiftNode,
        hdrSize);
...
TR::TreeTop *OMR::ValuePropagation::createArrayCopyVCallNodeAfterStores(TR::TreeTop *tree, TR::SymbolReference *srcRef,
    TR::SymbolReference *dstRef, TR::SymbolReference *lenRef, TR::SymbolReference *srcOffRef,
...
TR::TreeTop *createStoresForConverterCallChildren(TR::Compilation *comp, TR::TreeTop *callTree,
    TR::SymbolReference *&srcRef, TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef,
    TR::SymbolReference *&dstOffRef, TR::SymbolReference *&lenRef, TR::SymbolReference *&thisRef,
    TR::TreeTop *insertBefore)
{
...
TR::TreeTop *createStoresForArraycopyVCallChildren(TR::Compilation *comp, TR::TreeTop *vcallTree,
    TR::SymbolReference *&srcRef, TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef,
    TR::SymbolReference *&dstOffRef, TR::SymbolReference *&lenRef, TR::TreeTop *insertBefore)
{
...
void createAndInsertTestBlock(TR::Compilation *comp, TR::TreeTop *ifTree, TR::TreeTop *tree,
    TR::Block *insertAfterBlock, TR::Block *dstBlock)
{
    TR::CFG *cfg = comp->getFlowGraph();
...
TR::TreeTop *OMR::ValuePropagation::createSpineCheckNode(TR::Node *node, TR::SymbolReference *objSymRef)
{
    ...
    TR::Node *sizeNode = TR::Node::createWithSymRef(TR::iloadi, 1, 1, objNode,
...
//
TR::TreeTop *OMR::ValuePropagation::createAndInsertStoresForArrayCopySpineCheck(TR_ArrayCopySpineCheck *checkInfo)
{
...
TR::TreeTop *OMR::ValuePropagation::createArrayCopyCallForSpineCheck(TR_ArrayCopySpineCheck *checkInfo)
{
    ...
    if (acNode->getOpCodeValue() != TR::arraycopy) {
        ...
        //      TR_ASSERT((acNode->getOpCodeValue() == TR::arraycopy) || acNode->getOpCode().isStore(), "unexpected
        //      opcode");
    }
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy *rtArrayCopyTree)
{
    ...
    //------------------------------------------------
    // create ifNodes
    TR::Node *spineShiftNode
        = TR::Node::create(vcallNode, TR::iconst, 0, (int32_t)fe()->getArraySpineShift(elementSize));
    ...
    if (type != TR::Address || !comp()->getOptions()->realTimeGC()) {
        ...
        // srcOFf dstOff are only indicators for runtime tests needed, so make sure they are not null without
        ...
        TR::Node *fragmentParent = TR::Node::createWithSymRef(vcallNode, TR::aload, 0,
            comp()->getSymRefTab()->findOrCreateFragmentParentSymbolRef());
        if (comp()->target().is64Bit()) {
            TR::Node *globalFragment = TR::Node::createWithSymRef(TR::lloadi, 1, 1, fragmentParent,
                comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
            ifTree = TR::TreeTop::create(comp(),
                TR::Node::createif(TR::iflcmpne, globalFragment, TR::Node::create(vcallNode, TR::lconst, 0, 0)));
        } else {
            TR::Node *globalFragment = TR::Node::createWithSymRef(TR::iloadi, 1, 1, fragmentParent,
                comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
            ifTree = TR::TreeTop::create(comp(),
                TR::Node::createif(TR::ificmpne, globalFragment, TR::Node::create(vcallNode, TR::iconst, 0, 0)));
        }
    ...
    // if this is tarok, depending on wrtbar we create a primitive arraycopy or reference arraycopy with no store check
    // flag. for realtime the fast path is primitive (if we got there there is no need for wrtbar)
    if (!comp()->getOptions()->realTimeGC() && ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
        // tarok, need wrtbar
        generateRTArrayNodeWithoutFlags(rtArrayCopyTree, dupArraycopyTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef,
            0);
        ...
        if (comp()->useCompressedPointers()) {
            ...
            TR_ASSERT((dupArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the src arraylet pointer");
            TR_ASSERT((dupArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the dst arraylet pointer");

            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()));
            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()));
        }
        ...
        if (comp()->useCompressedPointers()) {
            TR::TreeTop *prevTree = dupArraycopyTree->getPrevTreeTop();
            TR_ASSERT((dupArraycopyTree->getNode()->getFirstChild()->getChild(0)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the src arraylet pointer");
            TR_ASSERT((dupArraycopyTree->getNode()->getFirstChild()->getChild(1)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the dst arraylet pointer");

            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupArraycopyTree->getNode()->getFirstChild()->getChild(0)->getFirstChild()));
            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupArraycopyTree->getNode()->getFirstChild()->getChild(1)->getFirstChild()));
        }
    }
    ...
    if (type == TR::Address && comp()->getOptions()->realTimeGC() && doMultiLeafArrayCopy) {
        ...
        slowArraycopyBlock = TR::Block::createEmptyBlock(origDupVCallTree->getNode(), comp(),
            origCallBlock->getFrequency(), origCallBlock);
        slowArraycopyBlock->append(origDupVCallTree);

        TR::Block *succ = fastArraycopyBlock->getNextBlock();
        slowArraycopyBlock->append(
            TR::TreeTop::create(comp(), TR::Node::create(origDupVCallTree->getNode(), TR::Goto, 0, succ->getEntry())));
    ...
    if ((type == TR::Address) && (rtArrayCopyTree->_flag & NEED_ARRAYSTORE_CHECK)) {
        // creating test for instanceof.
        // We know there is no wrtbar needed, so the fast path will be arraycopy with 3 children, while the slow will be
        // arraycopy with 5 children with arrayStore check.
        ...
        if (!comp()->getOptions()->realTimeGC() && ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
            dupRefArraycopyNode = TR::Node::createArraycopy(dupVCallNode->getChild(0)->duplicateTree(),
                dupVCallNode->getChild(2)->duplicateTree(), arraycopyNode->getChild(2)->duplicateTree(),
                arraycopyNode->getChild(3)->duplicateTree(), arraycopyNode->getChild(4)->duplicateTree());
        } else { // consturct from primitive
            dupRefArraycopyNode = TR::Node::createArraycopy(dupVCallNode->getChild(0)->duplicateTree(),
                dupVCallNode->getChild(2)->duplicateTree(), arraycopyNode->getChild(0)->duplicateTree(),
                arraycopyNode->getChild(1)->duplicateTree(), arraycopyNode->getChild(2)->duplicateTree());
        }
        ...
        TR::Block *referenceArraycopyBlock
            = TR::Block::createEmptyBlock(dupRefArraycopyNode, comp(), origCallBlock->getFrequency(), origCallBlock);
        if (!(rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC
                || rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_DST)) {
        ...
        if (comp()->useCompressedPointers()) {
            ...
            TR_ASSERT((dupRefArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the src arraylet pointer");
            TR_ASSERT((dupRefArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()->getOpCodeValue()
                          == TR::aloadi),
                "didn't find the dst arraylet pointer");

            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupRefArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()));
            prevTree = TR::TreeTop::create(comp(), prevTree,
                TR::Node::createCompressedRefsAnchor(
                    dupRefArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()));
        }

        TR::Block *succ = fastArraycopyBlock->getNextBlock();
        referenceArraycopyBlock->append(
            TR::TreeTop::create(comp(), TR::Node::create(dupArraycopyNode, TR::Goto, 0, succ->getEntry())));
...
void OMR::ValuePropagation::transformRTMultiLeafArrayCopy(TR_RealTimeArrayCopy *rtArrayCopyTree)
{
    ...
    logprintf(trace(), log, "Transforming multi-leaf array copy: %p\n", vcallNode);
    ...
    TR::Node *newCallNode = TR::Node::createWithSymRef(TR::call, 7, 7, vcallNode->getChild(0), vcallNode->getChild(1),
        vcallNode->getChild(2), vcallNode->getChild(3), vcallNode->getChild(4), extraChild1, extraChild2, helperSymRef);
    TR::TransformUtil::removeTree(comp(), vcallTree);
    TR::TreeTop *newCallTree = TR::TreeTop::create(comp(), prevTree, TR::Node::create(TR::treetop, 1, newCallNode));
...
void OMR::ValuePropagation::transformNullRestrictedArrayCopy(
    TR_NeedRuntimeTestNullRestrictedArrayCopy *nullRestrictedArrayCopy)
{
    /*
     *     Array Flattening Is NOT Enabled:
     *    ================================
 ...
 slowBlock-> n39n      BBStart <block_10> (freq 0) (cold)
    ...
    if (isNextBlockExtendedBlock) {
        if (isPrevBlockOfExtendedBlockEmpty) {
            nextBlock = tmpBlock;
            logprintf(trace(), log,
                "%s: prevBlockOfExtendedBlockEmpty 1 prevTT n%dn prevBlock block_%d nextTT n%dn nextBlock block_%d\n",
    ...
    if (trace()) {
        log->printf("%s: srcArrayRefNode n%dn %p dstArrayRefNode n%dn %p originBlock block_%d slowBlock block_%d "
        ...
        log->printf("%s: prevTT n%dn prevBlock block_%d nextTT n%dn nextBlock block_%d\n", __FUNCTION__,
            prevTT->getNode()->getGlobalIndex(), prevBlock->getNumber(), nextTT->getNode()->getGlobalIndex(),
            nextBlock->getNumber());
    }
    ...
    if (needTestDstArray) {
        ...
        TR::Node *ifNode
            = TR::Node::createif(TR::ificmpne, testIsNullRestrictedArray, TR::Node::iconst(0), slowBlock->getEntry());
...
static const char *transformedTargetName(TR::RecognizedMethod rm)
{
#ifdef J9_PROJECT_SPECIFIC
    switch (rm) {
 ...
 */
void OMR::ValuePropagation::transformObjectCloneCall(TR::TreeTop *callTree,
    OMR::ValuePropagation::ObjCloneInfo *cloneInfo)
{
    ...
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "inlineClone.location/object/(%s)", comp()->signature()), callTree);
    int32_t classNameLength;
    const char *className = TR::Compiler->cls.classNameChars(comp(), j9class, classNameLength);
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "inlineClone.type/(%s)/(%s)/%s", className, comp()->signature(),
    ...
    if (callTree->getNode()->getOpCode().isCheck()) {
        TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
            "it only makes sense to do this object clone transform for checks with a NULLCHK - how did we get here "
            "otherwise??");
        TR::Node *passthrough = TR::Node::create(callNode, TR::PassThrough, 1, objNode);
        callTree->insertBefore(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(callNode, callTree->getNode()->getOpCodeValue(), 1, passthrough,
                callTree->getNode()->getSymbolReference())));
    ...
    if (!cloneInfo->_isFixed) {
        ...
        TR::TreeTop *compareTree = TR::TreeTop::create(comp(),
            TR::Node::createif(TR::ifacmpne,
                TR::Node::createWithSymRef(callNode, TR::aloadi, 1, objNode,
                    comp()->getSymRefTab()->findOrCreateVftSymbolRef()),
                TR::Node::createWithSymRef(callNode, TR::loadaddr, 0,
                    comp()->getSymRefTab()->findOrCreateClassSymbol(
                        callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0, j9class)),
                slowPath->getEntry()));
    ...
    TR::Node *loadaddr = TR::Node::createWithSymRef(callNode, TR::loadaddr, 0,
        comp()->getSymRefTab()->findOrCreateClassSymbol(callNode->getSymbolReference()->getOwningMethodSymbol(comp()),
            0, j9class));
    TR::Node::recreateWithoutProperties(callNode, TR::New, 1,
        comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(
            objNode->getSymbolReference()->getOwningMethodSymbol(comp())));
    callNode->setAndIncChild(0, loadaddr);

    TR::SymbolReference *helperAccessor = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(),
        "com/ibm/jit/JITHelpers", "jitHelpers", "()Lcom/ibm/jit/JITHelpers;", TR::MethodSymbol::Static);
    TR::SymbolReference *objCopySymRef
        = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
            comp()->target().is64Bit() ? "unsafeObjectShallowCopy64" : "unsafeObjectShallowCopy32",
            comp()->target().is64Bit() ? "(Ljava/lang/Object;Ljava/lang/Object;J)V"
    ...
    objCopy->setAndIncChild(3,
        TR::Node::create(callNode, (comp()->target().is64Bit() ? TR::a2l : TR::a2i), 1, loadaddr));
 ...
 * https://github.com/eclipse-omr/omr/issues/6623
 */
void OMR::ValuePropagation::transformArrayCloneCall(TR::TreeTop *callTree,
    OMR::ValuePropagation::ArrayCloneInfo *cloneInfo)
{
    ...
    TR::DebugCounter::prependDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "inlineClone.location/array/(%s)", comp()->signature()), callTree);
    int32_t classNameLength;
    ...
    if (callTree->getNode()->getOpCode().isCheck()) {
        TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
            "it only makes sense to do this array clone transform for checks with a NULLCHK - how did we get here "
            "otherwise??");
        TR::Node *passthrough = TR::Node::create(callNode, TR::PassThrough, 1, objNode);
        callTree->insertBefore(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(callNode, callTree->getNode()->getOpCodeValue(), 1, passthrough,
    ...
    if (isPrimitiveClass) {
        ...
        if (disableSkipZeroInitInVP) {
            TR::SymbolReference *symRef = comp()->getSymRefTab()->findOrCreateNewArraySymbolRef(
                objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
        ...
        if (isFixedClass) {
            classNode = TR::Node::createWithSymRef(callNode, TR::loadaddr, 0,
                comp()->getSymRefTab()->findOrCreateClassSymbol(
                    callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0, j9class));
        } else {
            // Load the component class of the cloned array as 2nd child of anewarray as expected by the opcode.
            TR::Node *arrayClassNode = TR::Node::createWithSymRef(callNode, TR::aloadi, 1, objNode,
                comp()->getSymRefTab()->findOrCreateVftSymbolRef());
            classNode = TR::Node::createWithSymRef(callNode, TR::aloadi, 1, arrayClassNode,
                comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
        }
    ...
    lengthInBytes
        = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(comp(), lenNode, NULL, elementSize, false);
    ...
    if (isPrimitiveClass) {
        switch (elementSize) {
            case 1:
                ...
                break;
            case 2:
                arraycopy->setArrayCopyElementType(TR::Int16);
                break;
            case 4:
                arraycopy->setArrayCopyElementType(TR::Int32);
                break;
            case 8:
                arraycopy->setArrayCopyElementType(TR::Int64);
                break;
        }
...
void OMR::ValuePropagation::transformConverterCall(TR::TreeTop *callTree)
{
    ...
    if (!performTransformation(comp(), "%sChanging call %s [%p] to %s \n", OPT_DETAILS, callNode->getOpCode().getName(),
            callNode, transformedTargetName(rm)))
        return;
    ...
    createStoresForConverterCallChildren(comp(), callTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, thisRef,
        callTree);
    if (hasTable)
        tableNode->createStoresForVar(tableRef, callTree, true);

    // create arraytranslate and cold call
    generateArrayTranslateNode(callTree, arrayTranslateTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, tableRef,
        hasTable);
    createConverterCallNodeAfterStores(dupCallTree, callTree, srcRef, dstRef, lenRef, srcOffRef, dstOffRef, thisRef,
        tableRef);
    ...
    TR::Node *storeCall
        = TR::Node::createWithSymRef(TR::istore, 1, 1, dupCallTree->getNode()->getFirstChild(), symRefConvert);
    ...
    if (comp()->getOSRMode() != TR::involuntaryOSR && comp()->getHCRMode() != TR::osr) {
        TR::TreeTop *actt = TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(callNode, TR::asynccheck, 0,
                getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp()->getMethodSymbol())));
    ...
    TR::TreeTop *ifTreec = TR::TreeTop::create(comp(),
        TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, srcRef), TR::Node::aconst(callNode, 0)));
    createAndInsertTestBlock(comp(), ifTreec, callTree, origCallBlock, slowArraytranslateBlock);

    // Null check on desitantion
    TR::TreeTop *ifTreed = TR::TreeTop::create(comp(),
        TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, dstRef), TR::Node::aconst(callNode, 0)));
    ...
    if (comp()->target().cpu.isZ()) {
        ...
        // All converter methods except for sun_nio_cs_ext_SBCS_Encoder_encodeSBCS have default case threshold=0. The
        // names of all the methods still remain inside the switch for easier searchability in the codebase for these
        // converters and the prior analysis performed on them.
        switch (rm) {
        ...
        if (overrideThreshold) {
            ...
            logprintf(trace(), comp()->log(), "Overriding arraytranslate fallback threshold to %d\n", threshold);
        }

        if (threshold > 0) {
            TR::TreeTop *thresholdTestTree = TR::TreeTop::create(comp(),
                TR::Node::createif(TR::ificmplt, TR::Node::createLoad(callNode, lenRef),
                    TR::Node::create(TR::iconst, 0, threshold)));
...
int32_t TR::LocalValuePropagation::perform()
{
    if (unsigned(_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
        dumpOptDetails(comp(), "Can't do Local Value Propagation - too many nodes\n");
    } else {
...
int32_t TR::LocalValuePropagation::performOnBlock(TR::Block *block)
{
    ...
    //
    if (unsigned(_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
        dumpOptDetails(comp(), "Can't do Local Value Propagation on block %d - too many nodes\n", block->getNumber());
    } else {
        ...
        while (treeTop) {
            ...
            if ((blk != block) && !blk->isExtensionOfPreviousBlock())
                break;
...
const char *TR::LocalValuePropagation::optDetailString() const throw() { return "O^O LOCAL VALUE PROPAGATION: "; }
...
void OMR::ValuePropagation::launchNode(TR::Node *node, TR::Node *parent, int32_t whichChild)
{
    ...
    if (node && node->getVisitCount() != _visitCount) {
        int32_t valueNumber = getValueNumber(node);
        TR_ASSERT(valueNumber < _firstUnresolvedSymbolValueNumber,
            "value number too big, valueNumber:%d _firstUnresolvedSymbolValueNumber:%d", valueNumber,
            _firstUnresolvedSymbolValueNumber);
    ...
    if (newNode && newNode->getOpCodeValue() == TR::PassThrough) {
