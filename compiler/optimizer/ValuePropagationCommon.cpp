...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node)
 ...
 slowBlock---> n39n      BBStart <block_-1>
         ...
         if (trace())
            ...
            traceMsg(comp(), "%s: n%dn %p srcObjNode n%dn %p #%d dstObjNode n%dn %p #%d\n", __FUNCTION__, node->getGlobalIndex(), node,
               srcObjNode->getGlobalIndex(), srcObjNode, srcArrRefSymRef->getReferenceNumber(), dstObjNode->getGlobalIndex(), dstObjNode, dstArrRefSymRef->getReferenceNumber());
            }
