...
static void arrayCopy16BitPrimitiveEnhance1(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg,
                                            TR::Register* remainingSizeReg, TR::Register* tmpReg, TR::Register* indexReg,
                                            TR::CodeGenerator* cg, TR::LabelSymbol* mainEndLabel)
   {
   /*
   ...
   Each iteration copy qword (8 bytes)           (4 bytes)
                   |                                 |
                   |                                 |
   ...
   */
   ...
   if (trace)
      traceMsg(comp, "%s: node n%dn srcReg %s %d dstReg %s %d sizeReg %s %d\n", funcName, node->getGlobalIndex(),
         comp->getDebug()->getName(srcReg), srcReg->getKind(), comp->getDebug()->getName(dstReg), dstReg->getKind(), comp->getDebug()->getName(sizeReg), sizeReg->getKind());
  ...
  generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
   ...
   /*
       remainingSize = remainingSize - (size << 3)
    copyQwordLoopLabel:
       copy quadword from the source to the destination
       source = source + 8
       destination = destination + 8
       size = size - 1
       jne copyQwordLoopLabel
    */
   /*
       remainingSize = remainingSize - (size << 3)
       index = 0
    copyQwordLoopLabel:
       copy quadword from the (source + index * 8) to (destination + index * 8)
       index = index + 1
       size = size - 1
       jne copyQwordLoopLabel
   ...
   if (!disableReducedInstrunctions)
      ...
      generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg,
                                  generateX86MemoryReference(srcReg, indexReg, TR::MemoryReference::convertMultiplierToStride(8), 0, cg), cg);
      generateMemRegInstruction(TR::InstOpCode::S8MemReg, node,
                                  generateX86MemoryReference(dstReg, indexReg, TR::MemoryReference::convertMultiplierToStride(8), 0, cg), tmpReg, cg);
      ...
      generateRegImmInstruction(TR::InstOpCode::SHLRegImm1(), node, indexReg, 3, cg);
      ...
      generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg, generateX86MemoryReference(srcReg, 0, cg), cg);
      generateMemRegInstruction(TR::InstOpCode::S8MemReg, node, generateX86MemoryReference(dstReg, 0, cg), tmpReg, cg);
      generateRegImmInstruction(TR::InstOpCode::ADDRegImms(), node, srcReg, 8, cg);
      generateRegImmInstruction(TR::InstOpCode::ADDRegImms(), node, dstReg, 8, cg);
      generateRegImmInstruction(TR::InstOpCode::SUB1RegImm1, node, sizeReg, 1, cg);
      generateLabelInstruction(TR::InstOpCode::JNE4, node, copyQwordLoopLabel, cg);
      }
   ...
   // Process remainder
   /*
    copyDwordLabel:      // remaining size: 0, 2, 4, 6
       size = remainingSize >> 2
       jz copyWordLabel; // remaining size: 0, 2

       remainingSize = remainingSize - 4  // remaining size: 0, 2
...
static void arrayCopy16BitPrimitiveEnhance2(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg,
                                            TR::Register* remainingSizeReg, TR::Register* tmpReg, TR::Register* indexReg,
                                            TR::CodeGenerator* cg, TR::LabelSymbol* mainEndLabel)
   {
   /*
...
static void arrayCopy16BitPrimitiveEnhance3(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   {
   TR::Compilation *comp = cg->comp();
   bool trace = comp->getOption(TR_TraceCG);
   const char *funcName = "OMR::X86::TreeEvaluator::arrayCopy16BitPrimitiveEnhance3";
   if (trace)
   ...
   dependencies->addPreCondition(remainingSizeReg, TR::RealRegister::NoReg, cg);
   ...
   dependencies->addPostCondition(sizeReg, TR::RealRegister::ecx, cg);
   dependencies->addPostCondition(remainingSizeReg, TR::RealRegister::NoReg, cg);
   ...
   if (node->isForwardArrayCopy())
      {
      arrayCopy16BitPrimitiveEnhance1(node, dstReg, srcReg, sizeReg, remainingSizeReg, tmpReg, indexReg, cg, mainEndLabel);
      //arrayCopy16BitPrimitiveEnhance2(node, dstReg, srcReg, sizeReg, remainingSizeReg, tmpReg, indexReg, cg, mainEndLabel);
      }
   else // decide direction during runtime
      {
...
static void arrayCopy16BitPrimitive(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   ...
   TR::Compilation *comp = cg->comp();
   bool trace = comp->getOption(TR_TraceCG);
   const char *funcName = "OMR::X86::TreeEvaluator::arrayCopy16BitPrimitive";
   if (trace)
      traceMsg(comp, "%s: node n%dn srcReg %s %d dstReg %s %d sizeReg %s %d\n", funcName, node->getGlobalIndex(),
         comp->getDebug()->getName(srcReg), srcReg->getKind(), comp->getDebug()->getName(dstReg), dstReg->getKind(), comp->getDebug()->getName(sizeReg), sizeReg->getKind());
...
TR::Register *OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::Compilation *comp = cg->comp();
   bool trace = comp->getOption(TR_TraceCG);
   const char *funcName = "OMR::X86::TreeEvaluator::arraycopyEvaluator";

   if (trace)
      traceMsg(comp, "%s: node n%dn srcNode n%dn srcReg %s dstNode n%dn dstReg %s sizeNode n%dn elementSize %d %s\n", funcName,
         node->getGlobalIndex(), srcNode->getGlobalIndex(), comp->getDebug()->getName(srcReg),
         dstNode->getGlobalIndex(), comp->getDebug()->getName(dstReg), sizeNode->getGlobalIndex(), elementSize, TR::DataType::getName(dt));
...
#define shortConstArrayWithoutDirThreshold  16*4
   ...
   if (trace)
      traceMsg(comp, "%s: size %u useREPMOVSW %d forceByteArrayElementCopy %d optimizeForConstantLengthArrayCopy %d ignoreDirectionForConstantLengthArrayCopy %d isShortConstArrayWithDirection %d isShortConstArrayWithoutDirection %d isForwardArrayCopy %d isBackwardArrayCopy %d\n", funcName,
         size, useREPMOVSW, forceByteArrayElementCopy, optimizeForConstantLengthArrayCopy,
         ignoreDirectionForConstantLengthArrayCopy, isShortConstArrayWithDirection, isShortConstArrayWithoutDirection,
         node->isForwardArrayCopy(), node->isBackwardArrayCopy());

   if (isShortConstArrayWithDirection)
      {
      if (trace) traceMsg(comp, "%s: isShortConstArrayWithDirection 1 calling arraycopyForShortConstArrayWithDirection\n", funcName);
      ...
      {
      if (trace) traceMsg(comp, "%s: isShortConstArrayWithoutDirection 1 calling arraycopyForShortConstArrayWithoutDirection\n", funcName);
      ...
      if (cg->comp()->target().is64Bit() && !isSize64Bit)
         ...
         if (trace) traceMsg(comp, "%s: generateRegRegInstruction TR::InstOpCode::MOVZXReg8Reg4\n", funcName);
         }

      if (elementSize == 8 && cg->comp()->target().is32Bit())
         {
         if (trace) traceMsg(comp, "%s: calling arrayCopy64BitPrimitiveOnIA32\n", funcName);
         ...
         if (enableArrayCopy16BitPrimitiveEnhance)
            {
            arrayCopy16BitPrimitiveEnhance3(node, dstReg, srcReg, sizeReg, cg);
            }
         else
            {
         ...
         {
         if (trace) traceMsg(comp, "%s: calling arrayCopyDefault elementSize %d\n", funcName, elementSize);
