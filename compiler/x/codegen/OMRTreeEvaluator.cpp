...
static void arrayCopy16BitPrimitiveEnhanceImplP1Root16(TR::Node* node,
                                            TR::Register* dstReg,
                                            TR::Register* srcReg,
                                            TR::Register* sizeReg,
                                            TR::Register* tmpReg1,
   ...
   if (cg->comp()->getOption(TR_TraceCG))
      ...
      traceMsg(cg->comp(), "%s: node n%dn srcReg %s dstReg %s sizeReg %s REP_MOVS_THRESHOLD_BYTES %d\n", __FUNCTION__,
         node->getGlobalIndex(), cg->comp()->getDebug()->getName(srcReg), cg->comp()->getDebug()->getName(dstReg), cg->comp()->getDebug()->getName(sizeReg), REP_MOVS_THRESHOLD_BYTES);
      }
...
static void arrayCopy16BitPrimitiveEnhanceImplP2Root16(TR::Node* node,
                                            TR::Register* dstReg,
                                            TR::Register* srcReg,
                                            TR::Register* sizeReg,
                                            TR::Register* tmpReg1,
   ...
   generateRegImmInstruction(TR::InstOpCode::SUBRegImm4(), node, sizeReg, 2, cg);
   ...
   // ---------------------------------
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 16, cg);
   generateLabelInstruction(TR::InstOpCode::JAE4, node, copy18ORMoreBytesLabel, cg);
   ...
   generateLabelInstruction(TR::InstOpCode::label, node, copy4ORMoreBytesLabel, cg);
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 8, cg);
   generateLabelInstruction(TR::InstOpCode::JAE4, node, copy10ORMoreBytesLabel, cg);

   // 4, 6, 8 Bytes
   generateRegMemInstruction(TR::InstOpCode::L4RegMem, node, tmpReg1, generateX86MemoryReference(srcReg, sizeReg, 0, -2, cg), cg);
   generateRegMemInstruction(TR::InstOpCode::L4RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
   generateMemRegInstruction(TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -2, cg), tmpReg1, cg);
   generateMemRegInstruction(TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(dstReg, 0, cg), tmpReg2, cg);
   generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
   ...
   // 10-16 Bytes
   generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg1, generateX86MemoryReference(srcReg, sizeReg, 0, -6, cg), cg);
   generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
   generateMemRegInstruction(TR::InstOpCode::S8MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -6, cg), tmpReg1, cg);
   generateMemRegInstruction(TR::InstOpCode::S8MemReg, node, generateX86MemoryReference(dstReg, 0, cg), tmpReg2, cg);
   generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
   ...
   generateLabelInstruction(TR::InstOpCode::label, node, copy18ORMoreBytesLabel, cg);
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 32, cg);
   generateLabelInstruction(TR::InstOpCode::JAE4, node, copy34ORMoreBytesLabel, cg);

   // 18-32 Bytes
   generateRegMemInstruction(TR::InstOpCode::MOVDQURegMem, node, tmpXmmYmmReg1, generateX86MemoryReference(srcReg, sizeReg, 0, -14, cg), cg);
   generateRegMemInstruction(TR::InstOpCode::MOVDQURegMem, node, tmpXmmYmmReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
   generateMemRegInstruction(TR::InstOpCode::MOVDQUMemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -14, cg), tmpXmmYmmReg1, cg);
   generateMemRegInstruction(TR::InstOpCode::MOVDQUMemReg, node, generateX86MemoryReference(dstReg, 0, cg), tmpXmmYmmReg2, cg);
   generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
   ...
   // 34-64 Bytes
   generateRegMemInstruction(TR::InstOpCode::InstOpCode::VMOVDQUYmmMem, node, tmpXmmYmmReg1, generateX86MemoryReference(srcReg, sizeReg, 0, -30, cg), cg);
   generateRegMemInstruction(TR::InstOpCode::InstOpCode::VMOVDQUYmmMem, node, tmpXmmYmmReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
   generateMemRegInstruction(TR::InstOpCode::InstOpCode::VMOVDQUMemYmm, node, generateX86MemoryReference(dstReg, sizeReg, 0, -30, cg), tmpXmmYmmReg1, cg);
   generateMemRegInstruction(TR::InstOpCode::InstOpCode::VMOVDQUMemYmm, node, generateX86MemoryReference(dstReg, 0, cg), tmpXmmYmmReg2, cg);
   generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
   ...
   generateRegImmInstruction(TR::InstOpCode::ADDRegImm4(), node, sizeReg, 2, cg);
...
static void arrayCopy16BitPrimitiveEnhanceImplP3Root16(TR::Node* node,
                                            TR::Register* dstReg,
                                            TR::Register* srcReg,
                                            TR::Register* sizeReg,
                                            TR::Register* tmpReg1,
    ...
    */
   // ---------------------------------
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 14, cg);
   generateLabelInstruction(TR::InstOpCode::JA4, node, copy18ORMoreBytesLabel, cg);
   ...
   // ---------------------------------
   generateLabelInstruction(TR::InstOpCode::label, node, copy4ORMoreBytesLabel, cg);
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 6, cg);
   generateLabelInstruction(TR::InstOpCode::JA4, node, copy10ORMoreBytesLabel, cg);
   ...
   // ---------------------------------
   generateLabelInstruction(TR::InstOpCode::label, node, copy18ORMoreBytesLabel, cg);
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 30, cg);
   generateLabelInstruction(TR::InstOpCode::JA4, node, copy34ORMoreBytesLabel, cg);
   ...
   // ---------------------------------
   generateLabelInstruction(TR::InstOpCode::label, node, copy34ORMoreBytesLabel, cg);
   generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 62, cg);
   generateLabelInstruction(TR::InstOpCode::JA4, node, repMovsLabel, cg);
...
static void arrayCopy16BitPrimitiveEnhanceImplP4Root16(TR::Node* node,
                                            TR::Register* dstReg,
                                            TR::Register* srcReg,
                                            TR::Register* sizeReg,
                                            TR::Register* tmpReg1,
...
static void arrayCopy16BitPrimitiveEnhancement(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   {
   TR::Register* tmpReg1 = cg->allocateRegister(TR_GPR);
   TR::Register* tmpReg2 = cg->allocateRegister(TR_GPR);
   TR::Register* tmpXmmYmmReg1 = cg->allocateRegister(TR_FPR);
   TR::Register* tmpXmmYmmReg2 = cg->allocateRegister(TR_FPR);
   ...
   static bool enableP2Root16ZeroLengthSubtraction1 = feGetEnv("TR_EnableP2Root16ZeroLengthSubtraction1") != NULL;
   static bool enableP3Root16ZeroLengthSubtraction2 = feGetEnv("TR_EnableP3Root16ZeroLengthSubtraction2") != NULL;
   static bool enableP4Root16 = feGetEnv("TR_EnableP4Root16") != NULL;

   if (enableP2Root16ZeroLengthSubtraction1)
      arrayCopy16BitPrimitiveEnhanceImplP2Root16(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, mainEndLabel);
   else if (enableP3Root16ZeroLengthSubtraction2)
      arrayCopy16BitPrimitiveEnhanceImplP3Root16(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, mainEndLabel);
   else if (enableP4Root16)
      arrayCopy16BitPrimitiveEnhanceImplP4Root16(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, mainEndLabel);
   else
      arrayCopy16BitPrimitiveEnhanceImplP1Root16(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, mainEndLabel);
...
static void arrayCopy8BitPrimitive16root2(TR::Node *node,
                                          TR::Register *dstReg,
                                          TR::Register *srcReg,
                                          TR::Register *sizeReg,
                                          TR::Register *tmpReg1,
...
static void arrayCopy8BitPrimitive8root2(TR::Node *node,
                                          TR::Register *dstReg,
                                          TR::Register *srcReg,
                                          TR::Register *sizeReg,
                                          TR::Register *tmpReg1,
...
static void arrayCopy8BitPrimitive1(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   {
   TR::Register *tmpReg1 = cg->allocateRegister(TR_GPR);
   TR::Register *tmpReg2 = cg->allocateRegister(TR_GPR);
   ...
   static bool enableArrayCopy8BitPrimitiveTestZeroBeforeRepMovs = (feGetEnv("TR_EnableArrayCopy8BitPrimitiveTestZeroBeforeRepMovs") != NULL);
   if (enableArrayCopy8BitPrimitiveTestZeroBeforeRepMovs) // TODO-AHUO: E4, E5
      {
      generateRegImmInstruction(TR::InstOpCode::CMPRegImm4(), node, sizeReg, 0, cg);
      generateLabelInstruction(TR::InstOpCode::JE4, node, mainEndLabel, cg);
      }
...
static void arrayCopy8BitPrimitive2(TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   {
   TR::Register* tmpReg1 = cg->allocateRegister(TR_GPR);
   TR::Register* tmpReg2 = cg->allocateRegister(TR_GPR);
   TR::Register* tmpXmmYmmReg1 = cg->allocateRegister(TR_FPR);
   ...
   if (use8AsRoot) // TODO-AHUO: E6
      arrayCopy8BitPrimitive8root2(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsLabel, mainEndLabel);
   else // TODO-AHUO: E2
      arrayCopy8BitPrimitive16root2(node, dstReg, srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsLabel, mainEndLabel);
...
static void arrayCopyDefault(TR::Node* node, uint8_t elementSize, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
   {
   static bool disable8BitPrimitiveArrayCopyEnhancement = (feGetEnv("TR_Disable8BitPrimitiveArrayCopyEnhancement") != NULL);

   bool enableArrayCopy8BitPrimitiveEnhancement = ((elementSize == 1) &&
                                                   !disable8BitPrimitiveArrayCopyEnhancement &&
                                                   cg->comp()->target().cpu.supportsAVX() &&
                                                   cg->comp()->target().is64Bit()) ? true : false;


   if (enableArrayCopy8BitPrimitiveEnhancement)
      {
      static bool enableByteArrayTestZeroLength = (feGetEnv("TR_EnableByteArrayTestZeroLength") != NULL);

      if (enableByteArrayTestZeroLength)
         arrayCopy8BitPrimitive2(node, dstReg, srcReg, sizeReg, cg); // TODO-AHUO: E2 or E6
      else
         arrayCopy8BitPrimitive1(node, dstReg, srcReg, sizeReg, cg); // TODO-AHUO: E1 or E3
...
#define shortConstArrayWithoutDirThreshold  16*4
   ...
   if (isShortConstArrayWithDirection)
      ...
      if (elementSize == 8 && cg->comp()->target().is32Bit())
         ...
         {
         static bool disable16BitPrimitiveArrayCopyEnhancement = feGetEnv("TR_Disable16BitPrimitiveArrayCopyEnhancement") != NULL;

         bool enableArrayCopy16BitPrimitiveEnhancement = (!disable16BitPrimitiveArrayCopyEnhancement &&
                                                           cg->comp()->target().cpu.supportsAVX() &&
                                                           cg->comp()->target().is64Bit()) ? true : false;

         if (enableArrayCopy16BitPrimitiveEnhancement)
